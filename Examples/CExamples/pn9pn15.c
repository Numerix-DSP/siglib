...
int main (void)
{
    ...
    do {
        Tmp =
            SDS_SequenceGeneratorPN15 (&TxShiftRegister);   // Shift register
        if (SequenceLength < 10)                            // Print first 10 outputs to verify
        {
            printf ("Output = %x\n", (char)Tmp);
        }
    ...
    for (i = 0; i < 70; i++) {
        Tmp =
            SDS_ScramblerDescramblerPN9 (TxString[i],       // Source character
                                         &TxShiftRegister); // Shift register
        RxString[i] =
            (char)SDS_ScramblerDescramblerPN9 (Tmp,                 // Source character
    ...
    for (i = 0; i < 70; i++) {
        Tmp =
            SDS_ScramblerDescramblerPN15 (TxString[i],          // Source character
                                          &TxShiftRegister);    // Shift register
        RxString[i] =
            (char)SDS_ScramblerDescramblerPN15 (Tmp,                // Source character
                                                &RxShiftRegister);  // Shift register
    }
    RxString[70] = 0;                                   // Terminate string for printf
