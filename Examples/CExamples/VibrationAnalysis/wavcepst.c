  ...
  while ((sampleCount = SUF_WavReadData(pDataArray, fpInputFile, wavInfo, SAMPLE_LENGTH)) == SAMPLE_LENGTH) {
    ...
    while (SDA_CopyWithOverlap(pDataArray,               // Pointer to source array
      ...
      *(pFDPMagn + 1) = SIGLIB_ZERO;
      ...
      // SDA_LogMagnitude (pFDPMagn,                     // Pointer to real
      // source array pFDPPhase,                    // Pointer to imaginary
      // source array pFDPMagn,                     // Pointer to log magnitude
      // destination array pFDPMagn);                    // Array length
      // // Clip off noise
      // SDA_Add (pFDPMagn,                              // Pointer to source
      // array -50.0,                                 // Offset pFDPMagn, //
      // Pointer to destination array RESULT_LENGTH);                        //
      // Array length
      ...
      for (SLArrayIndex_t i = 0; i < RESULT_LENGTH; i++) {
        fprintf(fpOutputFile, "%lf\t%lf\t%lf\n", ((SLData_t)FrameNumber) * (((SLData_t)(SAMPLE_LENGTH - OVERLAP_LENGTH)) / SampleRate),
                (((SLData_t)i) * (SampleRate / ((SLData_t)FFT_LENGTH)) / 1000.0), *(pFDPMagn + i));
      }
