...
#define START_INDEX             90                          // Start index for peak search, in FFT output
                                                            ...
                                                            // Starting the track at a very low frequency is difficult because the signal is overwhelmed by noise
        ...
        while (SDA_CopyWithOverlap (pDataArray,             // Pointer to source array
            ...
            SDA_LogMagnitude (pFDPRealData,                 // Pointer to real source array
                              ...
                              RESULT_LENGTH);               // Array length
                                    ...
                                    // We only need to track small changes in the frequency domain, not across the entire spectrum
                                    // The window size allows us to track the highest rate of change anticipated
                                    // Then store the peak location in the PeakArray
                                    // The peak location is used in the resampling section to ensure the fundamental is tracked as the first order
...
SUF_Debugfprintf ("\t((SLData_t)*(pPeakArray+FrameNumber)) / 100.0 = %lf\n", ((SLData_t)*(pPeakArray+FrameNumber)) / 100.0);
            ...
            ResampleResultLength =
                SDA_ResampleSinc (pFDPRealData,             // Pointer to source array
...
#if COMMENT                     // Uncomment this section to reduce the background noise level in the plot
