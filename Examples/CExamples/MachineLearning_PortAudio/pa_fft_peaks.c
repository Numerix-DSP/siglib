...
void analog_isr (void)
{
    ...
    *(pInputData+Input_SamplesCount) = (double)adc_in0;
    ...
    Input_SamplesCount++;
    if (Input_SamplesCount == SAMPLE_LENGTH)                // If we have filled the array, mark data as valid and swap arrays
    {
...

{
    ...
    if (peakDetectCount == NUM_PEAKS_COUNT) {
        SDA_Add (pPeakDetect, -138.0, pPeakDetect, HALF_FFT_LENGTH);    // Offset graph results for 0 dB
