...
// Process channel 1 - just output zero
  ...
  Input_SamplesCount++;
  if (Input_SamplesCount == FFT_LENGTH)                             // If we have filled the array, mark data as valid and swap arrays
  {
...
// Allocate memory
  ...
  if ((pRealData == NULL) || (pImagData == NULL) || (pFFTCoeffs == NULL) ||
      (pWindowCoeffs == NULL) || (pOnePoleState == NULL) || (pLayer1PostActivation == NULL) ||
      (pLayer1PostActivation == NULL) || (pCombFilterStateArray == NULL)) {
