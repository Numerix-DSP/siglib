...
static SLArrayIndex_t VitDecSurvivorStateHistoryTable[TRACE_BACK_TABLE_LENGTH]
                                                     [SIGLIB_VITV32_NUMBER_OF_STATES];          // History table of survivor states
static SLArrayIndex_t MinQ4Q3[TRACE_BACK_TABLE_LENGTH][SIGLIB_VITV32_NUMBER_OF_PATH_STATES];    // Array to save minimium Q4Q3
                                                                                                // dibit history
  ...
  while (!ExitFlag) {
    ...
    for (SLFixData_t i = 0; i < INPUT_STRING_LENGTH; i++) {
      ...
      ViterbiOutput |= ((char)SDS_ViterbiDecoderV32(ChannelData[1],                                      // Source channel data
                                                    VitDecAccumulatedErrorTable,                         // Accumulated error array
                                                    (SLArrayIndex_t*)VitDecSurvivorStateHistoryTable,    // Survivor state history
                                                                                                         // table
                                                    &VitDecSurvivorStateHistoryOffset,                   // Offset into state history
                                                                                                         // array - this is used as a
                                                                                                         // circular pointer
                                                    (SLArrayIndex_t*)MinQ4Q3,                            // Q4Q3 History table
                                                    &RxDiffDecState,                                     // Previous output for differential decoder
                                                    &VitDecDoTraceBackFlag,                              // Flag is set to SIGLIB_TRUE when we are
                                                                                                         // in trace back mode
                                                    TRACE_BACK_DEPTH))
                       << 4;    // Trace back depth
