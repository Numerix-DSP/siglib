...
static SLData_t *pRealData, *pImagData, *pResults, *pFFTCoeffs, *pWindowCoeffs;
  ...
  void)
{
  ...
  h_GPC_Plot     *h2DPlotFreq;                                      // Plot object
  ...
  for (;;) {
    ...
    if (_kbhit ()) {
      ...
      if (action == 't') {
        ...
        if (ZXFlag == SIGLIB_P (double) (VEL_CROSS) {


            ZXFlag = SIGLIB_NEGATIVE_LEVEL_CROSS;}

            else {
            (double) (ZXFlag = SIGLIB_POSITIVE_LEVEL_CROSS;}
                      ...
                      Magnitude += INCREMENT_DECREMENT; if (Magnitude >= CLIP_LEVEL + 0.2) {
                      ...
                      gpc_close (h2DPlotTime); gpc_close (h2DPlotFreq); SUF_MemoryFree (pSrc);  // Free memory
                      SUF_MemoryFree (pRealData);
                      SUF_MemoryFree (pImagData); SUF_MemoryFree (pResults); SUF_MemoryFree (pFFTCoeffs); SUF_MemoryFree (pWindowCoeffs); exit (0);}
...
// Generate the carrier signal
                      ...
                      SDA_Clip (pSrc, pSrc, CLIP_LEVEL, SIGLIB_CLIP_BOTH, SAMPLE_LENGTH); SDA_Copy (pSrc, pRealData, SAMPLE_LENGTH);
...
// Calculate real power from complex
                      ...
                      printf ("Signal magnitude = %lf\n", Magnitude); previousMagnitude = Magnitude;}
