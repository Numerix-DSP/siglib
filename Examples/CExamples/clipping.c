// SigLib Example to demonstrate detection of clipping in time and frequency
...
int main(void)
{
  ...
  SIF_Fft(pFFTCoeffs,                 // Pointer to FFT coefficients
          SIGLIB_BIT_REV_STANDARD,    // Pointer to bit reverse address table - NOT
                                      // USED
  ...
  for (;;) {
    ...
    SDA_Rfft(pRealData,                  // Pointer to real array
             ...
             SIGLIB_BIT_REV_STANDARD,    // Pointer to bit reverse address table -
                                         // NOT USED
             FFT_LENGTH,                 // FFT length
    ...
    if (TriggerLocation > SAMPLE_LENGTH) {    // If no trigger detected, just
                                              // display from first sample
      TriggerLocation = 0;
    }

    if (TriggerLocation > DISPLAY_LENGTH) {    // If trigger is detected after mid point, just
                                               // display from mid point
