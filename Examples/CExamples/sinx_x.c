...
int main(void)
{
    ...
    for (i = 0, offset = SIGLIB_ONE, delay = NUM_ITERS-1;
         i < NUM_ITERS; i++, delay--, offset -= (SIGLIB_TWO / NUM_ITERS)) {
        ...
        SDA_Add (pData,                             // Pointer to source array
                 SDS_Pow (((32. * ((SLData_t)i) / ((SLData_t)NUM_ITERS)) - 16.), SIGLIB_TWO),   // D.C. Offset
                 pData,                             // Pointer to destination array
                 SAMPLE_LENGTH);                    // Dataset length
        SDA_Power (pData,                           // Pointer to source array
                   pData,                           // Pointer to destination array
