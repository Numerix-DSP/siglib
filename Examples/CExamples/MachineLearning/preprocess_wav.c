...
#define FFT_LENGTH                          256             // FFT length
...
#ifndef REMOVE_DC_IN_TIME_DOMAIN                            // Set to '1' to remove the D.C. component in the time domain, '0' otherwise
    ...
    #define LINEAR_FFT_MAGNITUDE            1               // Set to '1' to calculate linear FFT magnitude, '0' for log magnitude (dB)
        ...
        // #define QUANTIZE_TIME_DOMAIN_PEAK           ((SLData_t)SDS_Pow(SIGLIB_TWO, (SLData_t)(QUANTIZE_TIME_DOMAIN_NUM_BITS)))
        ...
        // #define QUANTIZE_FREQUENCY_DOMAIN_PEAK      ((SLData_t)SDS_Pow(SIGLIB_TWO, (SLData_t)(QUANTIZE_FREQUENCY_DOMAIN_NUM_BITS)))
...
int main (int argc, char *argv[])
{
    ...
    SLData_t    *pRealDataShortened = pRealData + networkFftStartBin;   // Shorten the arrays to the length of the Neural Network input
#if (!REAL_ONLY_FFT_OUTPUT)
    ...
    do {
        copyIndexLength =                                   // Copy with index allows for data augmentation by overlapping frames
        ...
        if (copyIndexLength == FFT_LENGTH) {                // If we have a full array to process then process it
            ...
            if (dataAugmentationRandomGainEnable == 1) {    // Data augmentation by applying random gain to input data
                SLData_t randomGain =
                    SDS_RandomNumber (dataAugmentationRandomGainMinimum, dataAugmentationRandomGainMaximum);
                randomGain =
                    SDS_dBmToLinear (randomGain,            // dBm input value
...

{
    ...
    printf ("    A stride length equal to the input frame length (or zero) is equivalent to 0 percent overlap\n\n");
    printf ("    A stride length equal to half the input frame length is equivalent to 50 percent overlap\n\n");
    printf ("The option to set the num values at the edge of the window frame to zero is useful for silicon implementations becasue it allows multipliers to skip sparse array values that are multiplied by zero.\n\n");
}
