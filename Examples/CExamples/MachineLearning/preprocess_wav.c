  ...
  void)
{
  ...
  printf
    ("\t-t threshold_level        Input threshold level. Frames with an absolute maximum level below the threshold will not be processed. (Optional)\n");
  ...
  printf
    ("The option to set the num values at the edge of the window frame to zero is useful for silicon implementations becasue it allows multipliers to skip sparse array values that are multiplied by zero.\n\n");
}
