...
int main(void)
{
    ...
    attackCoeff =
        SDS_OnePoleTimeConstantToFilterCoeff (ATTACK_MS_PERIOD, // Attack period in ms
                                              SAMPLE_RATE);     // Sample rate

    printf ("\nAttack period = %lf, Attack Coefficient = %lf\n", ATTACK_MS_PERIOD, attackCoeff);

    decayCoeff =
        SDS_OnePoleTimeConstantToFilterCoeff (DECAY_MS_PERIOD,  // Decay filter period in ms
                                              SAMPLE_RATE);     // Sample rate
...
#if ARRAY_OR_SAMPLE
    SDA_EnvelopeRMS (pSrc1,                         // Pointer to source array
    ...
    for (int i = 0; i < SAMPLE_LENGTH; i++) {
        pSrc1[i] =
            SDS_EnvelopeRMS (pSrc1[i],                  // Input sample
                             attackCoeff,               // Attack filter coefficient
                             decayCoeff,                // Decay filter coefficient
                             &envOnePoleFilterState);   // Pointer to one-pole state variable
    }
