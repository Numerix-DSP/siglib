...
static SLData_t ChannelData[SIGLIB_VITK3_SAMPLES_PER_BYTE];    // 16 coded samples for each 8
                                                               // bit input char

// Viterbi decoder persistent data
static SLArrayIndex_t VitDecSurvivorStateHistoryTable[TRACE_BACK_TABLE_LENGTH]
                                                     [SIGLIB_VITK3_NUMBER_OF_STATES];    // History table of survivor states
static SLData_t VitDecAccumulatedErrorTable[SIGLIB_VITK3_NUMBER_OF_STATES];              // Error accumulation table

static SLFixData_t VitDecOutputBitCount;                   // Counts the bits into the output word
                                                           // so they are correctly aligned
static SLChar_t VitDecDecodedByte;                         // This is where the decoded bits are built
                                                           // up into a byte
static SLArrayIndex_t VitDecSurvivorStateHistoryOffset;    // Offset into state history array - this
                                                           // is used as a circular pointer
static SLFixData_t VitDecDoTraceBackFlag;                  // Flag is set to SIGLIB_TRUE when we
...
int main(void)
{
  ...
  SLData_t Noise = SIGLIB_ZERO;    // Noise level to add to channel signal - 1.5
                                   // and 1.6 cause BER to increase

  SIF_ViterbiDecoderK3(&VitDecOutputBitCount,                               // Counts the bits into the output word so they are
                                                                            // correctly aligned
                       &VitDecDecodedByte,                                  // This is where the decoded bits are built up into a
                                                                            // byte
                       ...
                       &VitDecSurvivorStateHistoryOffset,                   // Offset into state history array -
                                                                            // this is used as a circular pointer
                       &VitDecDoTraceBackFlag,                              // Flag is set to SIGLIB_TRUE when we are in trace
                                                                            // back mode
  ...
  while (!ExitFlag) {
    ...
    for (SLArrayIndex_t i = 0; i < INPUT_STRING_LENGTH; i++) {
      ...
      ViterbiOutput = (char)SDS_ViterbiDecoderK3(ChannelData,                    // Source data pointer
                                                 &VitDecOutputBitCount,          // Counts the bits into the output word so they
                                                                                 // are correctly aligned
                                                 &VitDecDecodedByte,             // This is where the decoded bits are built up
                                                                                 // into a byte
                                                 VitDecAccumulatedErrorTable,    // Accumulated error array
                                                 (SLArrayIndex_t*)VitDecSurvivorStateHistoryTable,    // Survivor state history table
                                                 &VitDecSurvivorStateHistoryOffset,                   // Offset into state history array
                                                                                                      // - this is used as a circular
                                                                                                      // pointer
                                                 &VitDecDoTraceBackFlag,                              // Flag is set to SIGLIB_TRUE when we are in
                                                                                                      // trace back mode
                                                 TRACE_BACK_DEPTH);                                   // Trace back depth
