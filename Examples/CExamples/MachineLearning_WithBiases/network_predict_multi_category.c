...
static SLData_t layer2Weights[NUM_OUTPUT_NODES][NETWORK_HIDDEN_LAYER_NODES];
...
// Scale all samples to avoid overflow in exponent function
  ...
  for (SLArrayIndex_t predictionSequenceNumber = 0; predictionSequenceNumber < nRows; predictionSequenceNumber++) {
    SLNeuralNetworkPrediction_s prediction = SDA_TwoLayerNCategoryWithBiasesNetworkPredict (pPredictionData + (predictionSequenceNumber * NETWORK_INPUT_SAMPLE_LENGTH), // Pointer to data to predict
  ...
  void)
{
  ...
  printf
    ("\t-c category_number   Category number being searched for (Optional). If included then classifications with this number will be recorded as correct.\n");
