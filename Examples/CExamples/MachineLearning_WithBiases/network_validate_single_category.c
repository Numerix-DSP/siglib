...
// Scale all samples to avoid overflow in exponent function
  ...
  int             numItems = SUF_ReadWeightsWithBiasesBinaryFile (filenameWeights,  // Filename
                                                                  (SLData_t *) layer1Weights, // layer1Weights array
                                                                  layer1Biases, // layer1Biases array
                                                                  layer2Weights,  // layer2Weights array
                                                                  layer2Biases);  // layer2Biases array
  ...
  if (debugFlag == 1) {
    ...
    printf ("layer2Weights Min: %lf\n", SDA_Min ((SLData_t *) layer2Weights, NUM_OUTPUT_NODES * NETWORK_HIDDEN_LAYER_NODES));
    printf ("layer2Weights Max: %lf\n\n", SDA_Max ((SLData_t *) layer2Weights, NUM_OUTPUT_NODES * NETWORK_HIDDEN_LAYER_NODES));
  }
  ...
  for (SLArrayIndex_t validationSequenceNumber = 0; validationSequenceNumber < nRows; validationSequenceNumber++) {
    ...
    SLNeuralNetworkPrediction_s prediction = SDA_TwoLayer2CategoryWithBiasesNetworkPredict (pValidationData + (validationSequenceNumber * NETWORK_INPUT_SAMPLE_LENGTH), // Pointer to data to validate
