...
// Notes:
//    This program normalizes the peak input magnitude to 1.0 to support any desired input word length
//    The output data is scaled accordingly
  ...
  char *argv[])
{
  ...
  while ((sampleCount = SUF_WavReadData (inputData, fpInputFile, wavInfo, SAMPLE_FRAME_LENGTH)) == SAMPLE_FRAME_LENGTH) {
    SDA_Multiply (inputData, frontendGain, inputData, sampleCount); // Apply front-end gain
    ...
    SUF_WavWriteData (inputData, fpOutputFile, wavInfo, sampleCount); // Write successive blocks of data
    processedSampleCount += sampleCount;                            // Track number of samples written to save in .wav file header
  }
