...
int main(void)
{
  ...
  printf("%lf Peak value = %d Bits signed\n", PeakValue,
         (SLArrayIndex_t)SDS_PeakValueToBits(PeakValue,               // Peak value
  ...
  printf("%d Bits signed = %lf Peak value\n", (SLArrayIndex_t)NumberOfBits,
         SDS_BitsToPeakValue(NumberOfBits,            // Number of bits
                             SIGLIB_SIGNED_DATA));    // Sign type of the signal
  ...
  Linear = 32767.0;
  printf("%lf (%d bit signed) = %lf (dBm)\n", Linear, (SLArrayIndex_t)NumberOfBits,
         SDS_VoltageTodBm(Linear,            // Linear value
  ...
  printf("%lf (dBm) = %lf (%d bit signed)\n", dBm,
         SDS_dBmToVoltage(dBm,              // dBm value
                          ZerodBmLevel),    // Zero dBm Level
         (SLArrayIndex_t)NumberOfBits);
