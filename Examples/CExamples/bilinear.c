...
int main(void)
{
  ...
  NumberOfZeros = (SLArrayIndex_t)NUMBER_OF_POLES;    // If number of s-plane zeros < number of
                                                      // poles, additional zeros places at Fs/2
  ...
  for (SLArrayIndex_t i = 0; i < IIR_FILTER_STAGES; i++) {
    printf("%le, %le, %le, %le, %le\n", *(pIIRCoeffs + (i * SIGLIB_IIR_COEFFS_PER_BIQUAD)), *(pIIRCoeffs + 1 + (i * SIGLIB_IIR_COEFFS_PER_BIQUAD)),
           *(pIIRCoeffs + 2 + (i * SIGLIB_IIR_COEFFS_PER_BIQUAD)), *(pIIRCoeffs + 3 + (i * SIGLIB_IIR_COEFFS_PER_BIQUAD)),
           *(pIIRCoeffs + 4 + (i * SIGLIB_IIR_COEFFS_PER_BIQUAD)));
  }
