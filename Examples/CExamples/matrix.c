...
static const SLComplexRect_s SrcComplex2x2[] = {
    //  {3.0, 0.0},                                             // Use these
    //  values to test the real component of the matrix inverse function
    //  {1.0, 0.0},
    //  {4.0, 0.0},
    //  {2.0, 0.0},
    {0.0, 3.0},    // Use these values to test the imaginary component of the
...
int main(void)
{
  ...
  *pMatrixd++ = 4;
  ...
  // Invert matrix D
  SigLibErrorCode = SMX_Inverse(pMatrixd,                // Pointer to source matrix
  ...
  Determinant = SMX_Determinant(pMatrixd,                // Pointer to source matrix
                                pTempSourceArray,        // Temporary array for source
                                pRowInterchangeIndex,    // Row interchange indices
                                pScalingFactor,          // Scaling factor array
  ...
  printf("Source matrix = \n\t%lf + j%lf, %lf + j%lf\n\t%lf + j%lf, %lf + j%lf\n", SrcComplex2x2[0].real, SrcComplex2x2[0].imag,
         SrcComplex2x2[1].real, SrcComplex2x2[1].imag, SrcComplex2x2[2].real, SrcComplex2x2[2].imag, SrcComplex2x2[3].real, SrcComplex2x2[3].imag);
  SMX_ComplexInverse2x2(SrcComplex2x2, DstComplex2x2);
  printf("Destination matrix = \n\t%lf + j%lf, %lf + j%lf\n\t%lf + j%lf, %lf + "
         "j%lf\n",
         DstComplex2x2[0].real, DstComplex2x2[0].imag, DstComplex2x2[1].real, DstComplex2x2[1].imag, DstComplex2x2[2].real, DstComplex2x2[2].imag,
         DstComplex2x2[3].real, DstComplex2x2[3].imag);

#define DIAGONAL_MATRIX_DIMENSION 3

  SLData_t dsrc[DIAGONAL_MATRIX_DIMENSION][DIAGONAL_MATRIX_DIMENSION] = {{1., 2., 3.}, {4., 5., 6.}, {7., 8., 9.}};
