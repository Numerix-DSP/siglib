...
#if CHEB_HP
static const SLData_t   ChebyshevLPFCoeffs[] =
{
    1.0, 2.0, 1.0, -1.5548, 0.6493,
    1.0, 2.0, 1.0, -1.4996, 0.8482
};
#endif
...
int main (void)
{
    ...
    Scale =
        SDA_IirLpLpShift (ButterworthLPFCoeffs,     // Source coefficients
                          ModifiedCoeffs,           // Destination coefficients
                          0.2,                      // Frequency # 1
                          0.6,                      // Frequency # 2
                          SIGLIB_TWO,               // Sample rate
                          1);                       // Number of biquads
    printf ("Modified LPF Coeffs = %lf, %lf, %lf, %lf, %lf\n", ModifiedCoeffs[0], ModifiedCoeffs[1], ModifiedCoeffs[2], ModifiedCoeffs[3], ModifiedCoeffs[4]);
    printf ("Scale = %lf\n\n", Scale);
    ...
    Scale =
        SDA_IirLpHpShift (ChebyshevLPFCoeffs,       // Source coefficients
                          ModifiedCoeffs,           // Destination coefficients
                          0.2,                      // Frequency # 1
                          0.6,                      // Frequency # 2
                          SIGLIB_TWO,               // Sample rate
    ...
    printf ("Modified HPF Coeffs 1 = %lf, %lf, %lf, %lf, %lf\n", ModifiedCoeffs[0+5], ModifiedCoeffs[1+5], ModifiedCoeffs[2+5], ModifiedCoeffs[3+5], ModifiedCoeffs[4+5]);
