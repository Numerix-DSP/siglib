...
// kHz), with the output bandwidth (8 kHz) These filter coefficients were
// designed using Digital Filter Plus - https://numerix-dsp.com/dfplus/ Note:
// The input array length must be a multiple of the down conversion ratio
...
int main(int argc, char** argv)
{
  ...
  SUF_WavWriteHeader(fpOutputFile,
                     outputWavInfo);    // Write dummy header to output file
  ...
  while ((inputSampleCount = SUF_WavReadData(inputData, fpInputFile, inputWavInfo, SAMPLE_SIZE)) !=
         0) {    // Successively read arrays of upto SAMPLE_SIZE samples
    ...
    SUF_WavWriteData(outputData, fpOutputFile, outputWavInfo,
                     outputSampleCount);    // Write successive blocks of data
  ...
  SUF_WavWriteHeader(fpOutputFile,
                     outputWavInfo);    // Overwrite the header information
