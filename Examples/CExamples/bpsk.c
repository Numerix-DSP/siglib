...
#define CARRIER_SINE_TABLE_SIZE \
  ((SLArrayIndex_t)(SAMPLE_RATE_HZ / CARRIER_TABLE_FREQ))    // Number of samples in each of cos and
                                                             // sine table
...
int main(void)
{
  ...
  if ((NULL == pData) || (NULL == pCarrierTable) || (NULL == pCostasLpLPFCoeffs) || (NULL == pCostasLpLPF1State) || (NULL == pCostasLpLPF2State) ||
      (NULL == pVCOLookUpTable)) {

    printf("Memory allocation failure\n\n");
  }

#if DISPLAY_FILTER_OUTPUT
  SLData_t* pFilterOutput = SUF_VectorArrayAllocate(SAMPLE_LENGTH);    // Array for displaying output of LPF
  ...
  SIF_BpskDemodulate(&CostasLpVCOPhase,                // VCO phase
                     ...
                     &RxSampleSum);                    // Pointer to Rx sample sum - used to decide which bit was
                                                       // Tx'd
  ...
  for (LoopCount = 0; LoopCount < NUMBER_OF_LOOPS; LoopCount++) {
    ...
    for (SLArrayIndex_t i = 0; i < SAMPLE_LENGTH; i += (SIGLIB_BYTE_LENGTH * SYMBOL_LENGTH)) {
      for (SLArrayIndex_t j = 0; j < SIGLIB_BYTE_LENGTH; j++) {
#if DISPLAY_FILTER_OUTPUT
        RxDemodulatedBit = SDA_BpskDemodulateDebug(pData + i + (j * SYMBOL_LENGTH),    // Source array
                                                   ...
                                                   &RxSampleSum,                       // Pointer to Rx sample sum - used to decide which bit
                                                                                       // was Tx'd
                                                   SYMBOL_LENGTH,                      // Samples per symbol
        ...
        getchar();
#else
        RxDemodulatedBit = SDA_BpskDemodulate(pData + i + (j * SYMBOL_LENGTH),    // Source array
                                              &CostasLpVCOPhase,                  // VCO phase
                                              VCO_MODULATION_INDEX,               // VCO modulation index
                                              pVCOLookUpTable,                    // VCO look up table
                                              VCO_SINE_TABLE_SIZE,                // VCO look up table size
  ...
  *RxStringPtr = 0;                                  // Terminate string for printf
  printf("BPSK Received string: %s\n", RxString);    // Print received string
