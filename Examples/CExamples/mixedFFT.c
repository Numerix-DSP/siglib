...
//  Not transposed - requires the use of sample based final stages, which support non-contiguous datasets
//      In this mode, pptimized versions of radix-2 and radix-3 are used and only these two radices are supported
//      The radix-2 option allows the use of, for example, a radix-4 first stage, followed by a radix-2 final stage, for better performance
//      The radix-3 option allows the implementation of FFT sizes between those of the standard radix-2 FFT
...
int main(void)
{
    ...
    if ((NULL == pSrcData) || (NULL == pRealData) || (NULL == pImagData) ||
        (NULL == pRealResults) || (NULL == pImagResults) || (NULL == pFFTCoeffs)) {
   ...
   for (SLArrayIndex_t i = 0; i < FFT_LENGTH; i++) {
        ...
        SDA_Cfft2 (pRealResults+(i*OUTPUT_STAGE_FFT_LENGTH),    // Pointer to real source array
                   ...
                   pImagData+(i*OUTPUT_STAGE_FFT_LENGTH));      // Pointer to imaginary destination array
#else
