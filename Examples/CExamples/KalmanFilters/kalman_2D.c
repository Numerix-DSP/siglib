...
// This example compares the performance of the Kalman filter with a moving
// average (comb) filter
...
int main(int argc, char** argv)
{
  ...
  SDA_Copy(pPositionTrue, pPositionNoisy,
           SAMPLE_LENGTH);    // Copy to noisy signal
  ...
  // Generate noisy velocity signal - True velocity calculated by
  ...
  SDA_Copy(pVelocityTrue, pVelocityNoisy,
           SAMPLE_LENGTH);    // Copy to noisy signal
  ...
  // printf ("\n\nCovariance Matrix:\n");
  // SUF_PrintMatrix ((SLData_t *) kf.R,                   // Pointer to source
  // matrix 2,            // Number of rows 2);           // Number of columns
