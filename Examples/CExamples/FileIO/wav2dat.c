...
int main(int argc, char** argv)
{
  ...
  SUF_DatWriteHeader(fpOutputFile,
                     wavInfo.SampleRate);    // Write header - must be done ahead of writing data

  while ((inputSampleCount = SUF_WavReadData(pDataArray, fpInputFile, wavInfo, SAMPLE_SIZE)) !=
         0) {    // Successively read arrays of upto 128 samples
    outputSampleCount += inputSampleCount;
    sampleIndex += SUF_DatWriteData(pDataArray, fpOutputFile, wavInfo.SampleRate, sampleIndex, inputSampleCount);
  }
