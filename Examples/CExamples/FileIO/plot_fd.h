...
SLError_t plot_time_domain(SLData_t* pSrcReal, char* pLabelString, SLArrayIndex_t SequenceLength);
                                ...
                                SLArrayIndex_t DFTLength)
{
  ...
  MaxIndex = SDA_DetectFirstPeakOverThreshold(pFDPResults,       // Pointer to source array
                                              -20.0,             // Threshold over which peak will be detected
                                              DFTLength / 2);    // Dataset length

  if (MaxIndex == ((DFTLength / 2) - 1)) {     // If there is not a peak above the threshold
                                               // then detect the highest peak
    MaxIndex = SDA_MaxIndex(pFDPResults,       // Pointer to source array
                            DFTLength / 2);    // Dataset length
  }
...
SLError_t plot_frequency_magnitude(SLData_t* pSrcReal, SLData_t* pSrcImag, char* pLabelString, SLArrayIndex_t DFTLength)
{
  static h_GPC_Plot* h2DPlot;    // Plot object
  static int FirstTimeFlag = 1;
