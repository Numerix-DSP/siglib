...
#define GRAPHICAL_EQ_NUM_GAIN_LEVELS \
  ((SLArrayIndex_t)((GRAPHICAL_EQ_GAIN_MAX_DB - GRAPHICAL_EQ_GAIN_MIN_DB) / GRAPHICAL_EQ_GAIN_STEP_DB) + 1)    // Number of gain levels
...
int main(void)
{
  ...
  SIF_GraphicalEqualizerFilterBank(filterCutOffFrequencies,                 // Pointer to filter cut-off frequency array
                                   ...
                                   GRAPHICAL_EQ_NUM_GAIN_LEVELS);           // Number of gain levels for each frequency
                                                                            // band
