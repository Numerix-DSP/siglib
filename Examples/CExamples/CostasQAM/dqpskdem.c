...
#define MAX_CONST_POINTS_PER_BLOCK      52                      // Maximum number of output constellation points from SDA_CostasQamDemodulate = ((SAMPLE_LENGTH / SYMBOL_LENGTH) + SIGLIB_AI_ONE)
...
static SLData_t         *pELGRealOutputSynchDelay, *pELGImagOutputSynchDelay;   // ELG output real and imaginary synchronization delay
            ...
            for (i = 0; i < ProcessSampleLength; i++) {
                ...
                if ((RxSymbolCount == 1) && (((RealMagn*RealMagn)+(ImagMagn*ImagMagn)) > CONSTELLATION_THRESHOLD)) {
                    ...
                    ReceivedConstellationPoint =
                        SDA_MinIndex (ConstellationPointErrors, // Pointer to source array
                    ...
                    if (RxDiBit == 0x0)      SUF_Debugfprintf ("Dibits : 00");
                    else if (RxDiBit == 0x1) SUF_Debugfprintf ("Dibits : 01");
                    else if (RxDiBit == 0x2) SUF_Debugfprintf ("Dibits : 10");
                    else if (RxDiBit == 0x3) SUF_Debugfprintf ("Dibits : 11");
                    SUF_Debugfprintf ("\n");
#endif
