...
#define IMPULSE_LENGTH \
  10    // Impulse response dataset length
...
static const SLData_t Input[INPUT_LENGTH] = {    // Input data
    0.0, -0.1, -0.3, -0.45, -0.55, -0.45, -0.3, -0.1, 0.0,   0.1,  0.3,  0.6,  0.9, 1.0,
    0.9, 0.6,  0.3,  0.1,   0.0,   -0.1,  -0.2, -0.3, -0.35, -0.3, -0.2, -0.1, 0.0};
...
int main(void)
{
  ...
  if ((NULL == pDistortedInput) || (NULL == pSrcRealData) || (NULL == pSrcImagData) || (NULL == pImpulseRealData) || (NULL == pImpulseImagData) ||
      (NULL == pImpulseFdRealData) || (NULL == pImpulseFdImagData) || (NULL == pError) || (NULL == pFFTCoeffs)) {
  ...
  // Be careful with this because it can make the division unstable in the
  // deconvolution
  ...
  SDA_Subtract2(Input, pSrcRealData, pError,
                INPUT_LENGTH);    // Calculate difference between input and deconvolution
  ...
  gpc_plot_2d(h2DPlot,                       // Graph handle
              ...
              "red",                         // Colour
              GPC_ADD);                      // New graph
  printf("\nInput, Real and Imaginary Results and Error Between Input And "
         "Deconvolution\n");
