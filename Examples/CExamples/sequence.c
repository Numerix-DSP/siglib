...
// static SLFixData_t  NumSynchSequence = 0x0d554;      // 3, 1, 1, 1||1, 1, 1,
// 0 static SLFixData_t  NumSynchSequence = 0x0eaaa;      // 3, 2, 2, 2||2, 2,
// 2, 2
...
static SLFixData_t NumWordSynchState[NUMERICAL_SYNCH_SEQ_LEN];    // State array for bit synch.
                                                                  // detector
...
static SLChar_t CharSynchState[CHARACTER_SYNCH_SEQ_LENGTH];       // State array for character
                                                                  // synch. detector
...
int main(void)
{
  ...
  for (SLArrayIndex_t i = 0; i < NUMERICAL_SRC_SEQ_LEN; i++) {
    if (SDS_DetectNumericalBitSequence(NumSrcSequence[i],         // Input word
      ...
      printf("Numerical bit sequence synch detected at offset %d - Correct "
             "answer = 22\n",
             (int)i);
    }
  }
  ...
  for (SLArrayIndex_t i = 0; i < NUMERICAL_SRC_SEQ_LEN; i++) {
    if (SDS_DetectNumericalWordSequence(NumSrcSequence[i],          // Input word
                                        ...
                                        NUMERICAL_SYNCH_SEQ_LEN)    // Synchronization sequence length
        == SIGLIB_TRUE) {
      printf("Numerical word sequence synch detected at offset %d - Correct "
             "answer = 21\n",
             (int)i);
    }
  }
  ...
  for (SLArrayIndex_t i = 0; i < CHARACTER_SRC_SEQ_LEN; i++) {
    if (SDS_DetectCharacterSequence(CharSrcSequence[i],            // Input character
                                    ...
                                    CHARACTER_SYNCH_SEQ_LENGTH)    // Synchronization sequence length
        == SIGLIB_TRUE) {
      printf("Character sequence synch detected at offset %d - Correct answer "
             "= 21\n",
             (int)i);
    }
  }
