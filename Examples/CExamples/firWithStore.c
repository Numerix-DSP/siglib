// SigLib FIR Filter With Store Example
...
// Apply fir filter and store filtered data
  ...
  SDA_FirWithStore(pSrc,                        // Pointer to input array to be filtered
  ...
  SDA_FirWithStore(pSrc + SAMPLE_LENGTH / 2,    // Pointer to input array to be filtered
  ...
  for (SLArrayIndex_t i = 0; i < SAMPLE_LENGTH; i++) {
    *pDst++ = SDS_FirWithStore(*pSrc++,           // Input data sample to be filtered
                               pFilterState,      // Pointer to filter state array
                               pFilterTaps,       // Pointer to filter coefficients
                               FILTER_LENGTH);    // Filter length
  }
