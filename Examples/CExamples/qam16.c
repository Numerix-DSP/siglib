...
#define SAMPLE_LENGTH \
  ((SLFixData_t)512)             // Number of samples in array - should represent an integer
                                 // number of symbols
...
#if RRCF_ENABLE
#  define RX_STRING_NIBBLE_COUNT_START \
    1                                                 // Starting phase of Rx string nibble count
...
#define CARRIER_SINE_TABLE_SIZE \
  ((SLFixData_t)(SAMPLE_RATE_HZ / CARRIER_TABLE_FREQ))                                // Number of samples in each of cos and
                                                                                      // sine table
...
static SLArrayIndex_t TxSampleClock,
    RxSampleClock;                        // Used to keep track of the samples and symbols
...
static SLUInt32_t TxShiftRegister,
    RxShiftRegister;    // Must be at least 17 bits long
...
int main(void)
{
  ...
  // Allocate arrays
  SLData_t* pCarrierTable = SUF_QamCarrierArrayAllocate(CARRIER_SINE_TABLE_SIZE);    // Overlapped cosine + sine look-up table
  ...
  for (LoopCount = 0; LoopCount < NUMBER_OF_LOOPS; LoopCount++) {
...
#if DISPLAY_FREQ_DOMAIN
    {
      ...
      if (SigLibErrorCode != SIGLIB_NO_ERROR) {
        printf("\n\nSigLib failure, please ensure that you have enough "
               "memory\n\n");
