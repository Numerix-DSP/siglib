...
static SLData_t     *pRealDataCopy1, *pImagDataCopy1, *pRealDataCopy2, *pImagDataCopy2, *pRealDataCopy3, *pImagDataCopy3, *pRealDataCopy4, *pImagDataCopy4;
...
int main(void)
{
    ...
    if ((NULL == pRealData1) || (NULL == pImagData1) || (NULL == pRealData2) || (NULL == pImagData2) ||
        ...
        (NULL == pOutput2r1cResult1) || (NULL == pOutput2r1cResult2) ||
        (NULL == pFFTCoeffs)) {
    ...
    for (SLArrayIndex_t i = 0; i < HALF_FFT_LENGTH; i++) {
        printf ("SDA_Rfft #1 real [%d]: %lf, Array #1 real [%d]: %lf, Error: %lf\n", i, pRealData1[i], i, pOutput2r1cResult1[i], pRealData1[i]-pOutput2r1cResult1[i]);
    }
    printf("Array #1 Imaginary\n");
    for (SLArrayIndex_t i = 0; i < HALF_FFT_LENGTH; i++) {
        printf ("SDA_Rfft #1 imag [%d]: %lf, Array #1 imag [%d]: %lf, Error: %lf\n", i, pImagData1[i], i, pOutput2r1cResult1[HALF_FFT_LENGTH+i], pImagData1[i]-pOutput2r1cResult1[HALF_FFT_LENGTH+i]);
    }
    printf("\n");

    printf("Array #2 Real\n");
    for (SLArrayIndex_t i = 0; i < HALF_FFT_LENGTH; i++) {
        printf ("SDA_Rfft #2 real [%d]: %lf, Array #2 real [%d]: %lf, Error: %lf\n", i, pRealData2[i], i, pOutput2r1cResult2[i], pRealData2[i]-pOutput2r1cResult2[i]);
    }
    printf("Array #2 Imaginary\n");
    for (SLArrayIndex_t i = 0; i < HALF_FFT_LENGTH; i++) {
        printf ("SDA_Rfft #2 imag [%d]: %lf, Array #2 imag [%d]: %lf, Error: %lf\n", i, pImagData2[i], i, pOutput2r1cResult2[HALF_FFT_LENGTH+i], pImagData2[i]-pOutput2r1cResult2[HALF_FFT_LENGTH+i]);
    }
    printf("\n");
