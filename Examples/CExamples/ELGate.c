...
//
// Loop filters
// In many early-late gate synchronizers it is necessary to have separate loop
// filters for acquisition mode and tracking mode. This function can support
// different filter specifications by passing separate filter coefficient arrays
// to the function at run time. In tracking mode a wider bandwidth is required
// to provide a faster transient response, while tracking mode requires a
// narrower bandwidth to provide greater stability. The coefficients of these
// different filters can be designed using the SIF_FirLowPassFilter function.
//
// Output synchronization delay
// The filters within the early-late gate timing error detector cause delays
// that effect the timing of the output synchronization pulse so it is important
// to account for this by delaying the signal that needs to be sampled. If the
...
int main(void)
{
  ...
  static SLFixData_t ELGPulseDetectorThresholdFlag;    // Flag to indicate signal level higher
                                                       // than noise level
  static SLData_t ELGZeroCrossingPreviousSample;       // Previous sample used for
  ...
  static SLData_t loopFilterOutput[SAMPLE_LENGTH];    // Arrays for storing output
                                                      // of debug version of early
                                                      // late gate synchronizer
  ...
  if ((NULL == pSrc) || (NULL == pDelayedSrc) || (NULL == pTriggerOutput) || (NULL == pELGMatchedFilterState) || (NULL == pELGEarlyGateDelay) ||
      (NULL == pELGLoopFilterCoeffs) || (NULL == pELGLoopFilterState) || (NULL == pELGOutputSynchDelay)) {
...
#  endif
  ...
  SDA_SignalGenerate(pSrc + START_DELAY,              // Pointer to destination array
                     ...
                     SIGLIB_ZERO,                     // Signal end value - Unused
                     &gaussPhase,                     // Pointer to gaussian signal phase - should be initialised
                                                      // to zero
                     &gaussValue,                     // Gaussian signal second sample - should be initialised to
                                                      // zero
