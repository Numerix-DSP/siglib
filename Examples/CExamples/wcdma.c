...
#define ADD_JITTERANDGAUSSIAN_NOISE \
  1    // Set to '1' to add jitter and Gaussian noise to the output signal, '0'
       // otherwise
...
int main(int argc, char* argv[])
{
  ...
  for (SLArrayIndex_t LoopCount = 0; LoopCount < NumberOfIterations; LoopCount++) {
    ...
    DataOutBits = SDA_ComplexQPSKDeSpread(DataOut,               // Pointer to destination array
                                          ChannelizationCode,    // In-phase channelization code
                                          ChannelizationCode,    // Quadrature-phase channelization code
  ...
  if (BitErrorCount == (SLFixData_t)0) {
    ...
    printf("FAIL !\n");
    printf("Total number of bits = %d, Number of bits in error = %d, Bit error "
           "rate = %lf\n",
