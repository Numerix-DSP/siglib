...

{
    ...
    SUF_DatWriteHeader (fpOutputFile, sampleRate);          // Write header - must be done ahead of writing data
    sampleIndex += SUF_DatWriteData (srcData, fpOutputFile, sampleRate, sampleIndex, IO_LENGTH);      // Write successive blocks of data
    ...
    while ((SampleCount = (int)SUF_DatReadData (inputData, fpInputFile, IO_LENGTH)) == IO_LENGTH) {
