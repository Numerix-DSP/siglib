...
CSLTestWindow::CSLTestWindow()
{
    ...
    ClassName = AfxRegisterWndClass (CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW,
        ...
        SLTestApp.LoadIcon (AFX_IDI_STD_FRAME));

    Create (ClassName, "SLTest", WS_OVERLAPPEDWINDOW, rectDefault, NULL, "SLTestMenu");

                                // Initialise memory
    if (Signal.GetSize() == 0) {
        HDSignalLocalMem = ::LocalAlloc (LMEM_MOVEABLE | LMEM_ZEROINIT,
            ((UINT)(FFT_LENGTH * DECIMATE_RATIO * sizeof (SLData_t))));
        HDFreqRLocalMem = ::LocalAlloc (LMEM_MOVEABLE | LMEM_ZEROINIT,
            ((UINT)(FFT_LENGTH * sizeof (SLData_t))));
        HDFreqILocalMem = ::LocalAlloc (LMEM_MOVEABLE | LMEM_ZEROINIT,
            ((UINT)(FFT_LENGTH * sizeof (SLData_t))));
        HDResultLocalMem = ::LocalAlloc (LMEM_MOVEABLE | LMEM_ZEROINIT,
            ((UINT)(FFT_LENGTH * sizeof (SLData_t))));
        HDCombFilter1LocalMem = ::LocalAlloc (LMEM_MOVEABLE | LMEM_ZEROINIT,
            ((UINT)(DECIMATION_FILTER_LENGTH * sizeof (SLData_t))));
        HDCombFilter2LocalMem = ::LocalAlloc (LMEM_MOVEABLE | LMEM_ZEROINIT,
            ((UINT)(DECIMATION_FILTER_LENGTH * sizeof (SLData_t))));
        HDSineTableLocalMem = ::LocalAlloc (LMEM_MOVEABLE | LMEM_ZEROINIT,
            ((UINT)(SINE_BUF_SIZE * sizeof (SLData_t))));
        HDFFTCoeffLocalMem = ::LocalAlloc (LMEM_MOVEABLE | LMEM_ZEROINIT,
            ((UINT)(((FFT_LENGTH * 3) / 4) * sizeof (SLData_t))));
    }
    else {
        HDSignalLocalMem = ::LocalReAlloc (Signal.GetHandle(),
            ((UINT)(FFT_LENGTH * DECIMATE_RATIO * sizeof (SLData_t))), LMEM_MOVEABLE);
        HDFreqRLocalMem = ::LocalReAlloc (FreqR.GetHandle(),
            ((UINT)(FFT_LENGTH * sizeof (SLData_t))), LMEM_MOVEABLE);
        HDFreqILocalMem = ::LocalReAlloc (FreqI.GetHandle(),
            ((UINT)(FFT_LENGTH * sizeof (SLData_t))), LMEM_MOVEABLE);
        HDCombFilter1LocalMem = ::LocalReAlloc (CombFilter1.GetHandle(),
            ((UINT)(DECIMATION_FILTER_LENGTH * sizeof (SLData_t))), LMEM_MOVEABLE);
        HDCombFilter2LocalMem = ::LocalReAlloc (CombFilter2.GetHandle(),
            ((UINT)(DECIMATION_FILTER_LENGTH * sizeof (SLData_t))), LMEM_MOVEABLE);
        HDSineTableLocalMem = ::LocalReAlloc (SineTable.GetHandle(),
            ((UINT)(SINE_BUF_SIZE * sizeof (SLData_t))), LMEM_MOVEABLE);
        HDFFTCoeffLocalMem = ::LocalReAlloc (FFTCoeff.GetHandle(),
            ((UINT)(((FFT_LENGTH * 3) / 4) * sizeof (SLData_t))), LMEM_MOVEABLE);
    }
...
afx_msg void CSLTestWindow::OnPaint()
{
    ...
    PaintDC.TextOut ((Rect.right * 3) / 32, ((Rect.bottom / 20) - 10),
        String1, String1.GetLength());
                    // Instead of 10, use SLChar_t height / 2

    PaintDC.TextOut ((Rect.right * 3) / 32,
        ((Rect.bottom * 3 / 8) + (Rect.bottom / 20) - 10),
        String2, String2.GetLength());

    PaintDC.TextOut ((Rect.right * 3) / 32,
        ((Rect.bottom * 3 / 4) + (Rect.bottom / 20) - 10),
        String3, String3.GetLength());
}
...
afx_msg void CSLTestWindow::OnZoomFFTSignal()
{
    ...
    if (FirstTime) {
        SIF_ZoomFftSimple (CombFilter1Ptr, &CombFilter1Sum, CombFilter2Ptr, &CombFilter2Sum, &CombFilterPhase,
            SineTablePtr, &SineTablePhase, FFTCoeffPtr, SIGLIB_NULL_ARRAY_INDEX_PTR,
            DECIMATION_FILTER_LENGTH, SINE_BUF_SIZE, FFT_LENGTH);
        FirstTime = 0;
    }


                    /* Perform Zoom FFT */
    SDA_ZoomFftSimple (SignalBufferPtr, FreqRBufferPtr, FreqIBufferPtr,
        CombFilter1Ptr, &CombFilter1Sum, CombFilter2Ptr, &CombFilter2Sum,
        &CombFilterPhase, SineTablePtr, &SineTablePhase, CARRIER_FREQUENCY,
        DECIMATION_FILTER_LENGTH, SINE_BUF_SIZE, DECIMATE_RATIO,
        FFTCoeffPtr, SIGLIB_NULL_ARRAY_INDEX_PTR,
        SOURCE_BUF_SIZE, FFT_LENGTH, ((SLArrayIndex_t)(SDS_Log2(FFT_LENGTH)+SIGLIB_MIN_THRESHOLD))   // Log2 FFT length, and avoid quantization issues);
