...
int analog_open (int SampleRate, void (*analog_isr) (void))
{
    ...
    printf ("Connected to input device %d : %s, inputs = %d, outputs = %d, Host API = %s\n", inputParameters.device, deviceInfo->name, (int)deviceInfo->maxInputChannels, (int)deviceInfo->maxOutputChannels,  Pa_GetHostApiInfo( deviceInfo->hostApi )->name);
    deviceInfo = Pa_GetDeviceInfo( outputParameters.device );
    printf ("Connected to output device %d: %s, inputs = %d, outputs = %d, Host API = %s\n", outputParameters.device, deviceInfo->name, (int)deviceInfo->maxInputChannels, (int)deviceInfo->maxOutputChannels,  Pa_GetHostApiInfo( deviceInfo->hostApi )->name);
    return 0;
