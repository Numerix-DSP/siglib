  ...
  char *st)
{
  ...
  if (cb->bufferDepth != 0) {                                       // Check if circular buffer NOT empty
    memcpy (st, cb->buf + cb->readIndex * cb->max_string_len, cb->max_string_len);  // Read string from circular buffer
  ...
  char *st)
{
  ...
  if (cb->bufferDepth != cb->size) {                                // Check if circular buffer NOT full
    memcpy (cb->buf + cb->writeIndex * cb->max_string_len, st, cb->max_string_len); // Write string to circular buffer
