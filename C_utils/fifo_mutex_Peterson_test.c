// Circular buffer for strings protected by Peterson’s two process mutual
...
// discarded once the fifo overflows Cygwin : gcc fifo_mutex_Peterson_test.c
// -lpthread -o fifo_mutex_Peterson_test.exe Linux  : gcc
// fifo_mutex_Peterson_test.c -lpthread -o fifo_mutex_Peterson_test This
// software is supplied under the terms of the MIT license:
...
void* producer_thread(void* arg)
{
  ...
  cb_string_init(&myCircularBuffer, MAX_CB_STRING_LENGTH, circularBuffer,
                 CIRCULAR_BUFFER_SIZE);    // Initialize the circular buffer

  printf("Producer thread started\n");

  while (1) {
    sprintf(inputString, "Message : %d",
            messageNumber++);    // Create message to send to consumer and
    ...
    if (*globalStringResource == 0) {    // If global resource empty send next
                                         // pending message in circular buffer
