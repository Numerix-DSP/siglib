...
SLError_t SIGLIB_FUNC_DECL SIF_Fft16 (
  SLInt16_t SIGLIB_PTR_DECL *,                                      // FFT coeff pointer
  const SLArrayIndex_t);                                            // FFT length

void SIGLIB_FUNC_DECL SDA_Rfft16 (
  SLInt16_t SIGLIB_PTR_DECL *,                                      // Real data array pointer
...
SLError_t SIGLIB_FUNC_DECL SIF_Fft32 (
  SLInt32_t SIGLIB_PTR_DECL *,                                      // FFT coeff pointer
  const SLArrayIndex_t);                                            // FFT length

void SIGLIB_FUNC_DECL SDA_Rfft32 (
  SLInt32_t SIGLIB_PTR_DECL *,                                      // Real data array pointer
  SLInt32_t SIGLIB_PTR_DECL *,                                      // Imaginary data array pointer
  const SLInt32_t SIGLIB_PTR_DECL *,                                // FFT coeff pointer
  const SLArrayIndex_t,                                             // FFT length
  const SLArrayIndex_t);                                            // log2 FFT length

SLError_t SIGLIB_FUNC_DECL SIF_Window16 (
  SLInt16_t * SIGLIB_PTR_DECL pWindowCoeffs,
  const enum SLWindow_t WindowType,
  const SLData_t Coeff,
  const SLArrayIndex_t);

void SIGLIB_FUNC_DECL SDA_Window16 (
  const SLInt16_t * SIGLIB_PTR_DECL pSrc,
  SLInt16_t * SIGLIB_PTR_DECL pDst,
  const SLInt16_t * SIGLIB_PTR_DECL pWindowCoeffs,
  const SLArrayIndex_t);

SLError_t SIGLIB_FUNC_DECL SIF_Window32 (
  SLInt32_t * SIGLIB_PTR_DECL pWindowCoeffs,
  const enum SLWindow_t WindowType,
  const SLData_t Coeff,
  const SLArrayIndex_t);

void SIGLIB_FUNC_DECL SDA_Window32 (
  const SLInt32_t * SIGLIB_PTR_DECL pSrc,
  SLInt32_t * SIGLIB_PTR_DECL pDst,
  const SLInt32_t * SIGLIB_PTR_DECL pWindowCoeffs,
  const SLArrayIndex_t);

void SIGLIB_FUNC_DECL SUF_Fix16ToSigLibData (
  SLInt16_t * SIGLIB_PTR_DECL pSrc,                                 // Pointer to source array
  SLData_t * SIGLIB_PTR_DECL pDst,                                  // Pointer to destination array
  const SLArrayIndex_t);                                            // Array length

void SIGLIB_FUNC_DECL SUF_SigLibDataToFix16 (
  SLData_t * SIGLIB_PTR_DECL pSrc,                                  // Pointer to source array
  SLInt16_t * SIGLIB_PTR_DECL pDst,                                 // Pointer to destination array
  const SLArrayIndex_t);                                            // Array length

void SIGLIB_FUNC_DECL SUF_Fix32ToSigLibData (
  SLInt32_t * SIGLIB_PTR_DECL pSrc,                                 // Pointer to source array
  SLData_t * SIGLIB_PTR_DECL pDst,                                  // Pointer to destination array
  const SLArrayIndex_t);                                            // Array length

void SIGLIB_FUNC_DECL SUF_SigLibDataToFix32 (
  SLData_t * SIGLIB_PTR_DECL pSrc,                                  // Pointer to source array
...
//  SDA_SignalGenerate (pFloatSrcData, SIGLIB_SINE_WAVE, ((SLData_t)SIGNAL_MAGNITUDE), SIGLIB_FILL, (SIGLIB_ONE / ((SLData_t)FFT_SIZE)), SIGLIB_ZERO,
//  SDA_SignalGenerate (pFloatSrcData, SIGLIB_SINE_WAVE, ((SLData_t)SIGNAL_MAGNITUDE), SIGLIB_FILL, (SIGLIB_TWO / ((SLData_t)FFT_SIZE)), SIGLIB_ZERO,
//  SDA_SignalGenerate (pFloatSrcData, SIGLIB_SINE_WAVE, ((SLData_t)SIGNAL_MAGNITUDE), SIGLIB_FILL, (SIGLIB_FOUR / ((SLData_t)FFT_SIZE)), SIGLIB_ZERO,
//  SDA_SignalGenerate (pFloatSrcData, SIGLIB_COSINE_WAVE, ((SLData_t)SIGNAL_MAGNITUDE), SIGLIB_FILL, (SIGLIB_ONE / ((SLData_t)FFT_SIZE)), SIGLIB_ZERO,
//  SDA_SignalGenerate (pFloatSrcData, SIGLIB_COSINE_WAVE, ((SLData_t)SIGNAL_MAGNITUDE), SIGLIB_FILL, (SIGLIB_TWO / ((SLData_t)FFT_SIZE)), SIGLIB_ZERO,
  ...
  SUF_SigLibDataToFix16 (pFloatSrcData, pRealData, FFT_SIZE);       // Convert the SigLib data to fixed point
#if ENABLE_WINDOWING
  SigLibError = SIF_Window16 (pWindowCoeffs, SIGLIB_HANNING, SIGLIB_ZERO, FFT_SIZE);  // Generate Hanning window table
  if (SigLibError != SIGLIB_NO_ERROR)
    ExitOnError ("SIF_Window16");
  SDA_Window16 (pRealData, pRealData, pWindowCoeffs, FFT_SIZE);     // Apply window to real data
#endif
#if DEBUG
#if ENABLE_WINDOWING
  ...
  if (SigLibError != SIGLIB_NO_ERROR)
    ExitOnError ("SIF_Fft16");
  SUF_SigLibDataToFix32 (pFloatSrcData, pRealData, FFT_SIZE);       // Convert the SigLib data to fixed point
#if ENABLE_WINDOWING
  SigLibError = SIF_Window32 (pWindowCoeffs, SIGLIB_HANNING, SIGLIB_ZERO, FFT_SIZE);  // Generate Hanning window table
  if (SigLibError != SIGLIB_NO_ERROR)
    ExitOnError ("SIF_Window32");
  SDA_Window32 (pRealData, pRealData, pWindowCoeffs, FFT_SIZE);     // Apply window to real data
#endif
#if DEBUG
#if ENABLE_WINDOWING
  for (i = 0; i < FFT_SIZE; i++)
...
SLError_t SIGLIB_FUNC_DECL SIF_Fft16 (
  SLInt16_t * SIGLIB_PTR_DECL pFFTCoeffs,
  const SLArrayIndex_t FFTLength)
{
  SLArrayIndex_t  i;
...
void SIGLIB_FUNC_DECL SDA_Rfft16 (
  SLInt16_t SIGLIB_PTR_DECL realData[],
  SLInt16_t SIGLIB_PTR_DECL imagData[],
  const SLInt16_t * SIGLIB_PTR_DECL pFFTCoeffs,
  const SLArrayIndex_t FFTLength,
...
SLError_t SIGLIB_FUNC_DECL SIF_Fft32 (
  SLInt32_t * SIGLIB_PTR_DECL pFFTCoeffs,
  const SLArrayIndex_t FFTLength)
{
  SLArrayIndex_t  i;
...
void SIGLIB_FUNC_DECL SDA_Rfft32 (
  SLInt32_t SIGLIB_PTR_DECL realData[],
  SLInt32_t SIGLIB_PTR_DECL imagData[],
  const SLInt32_t * SIGLIB_PTR_DECL pFFTCoeffs,
  const SLArrayIndex_t FFTLength,
  const SLArrayIndex_t Log2Size)
{
  ...
  const SLArrayIndex_t WindowLength)
{
  ...
  SigLibErrorCode = SIF_Window (pTmp, WindowType, Coeff, WindowLength); // Initialise the window coefficients
  ...
  SDA_Multiply (pTmp, (SLData_t) SIGLIB_SHORT_WORD_MAX, pTmp, WindowLength);  // Scale the coefficients
...
void SIGLIB_FUNC_DECL SUF_Fix16ToSigLibData (
  SLInt16_t * SIGLIB_PTR_DECL pSrc,
...
void SIGLIB_FUNC_DECL SUF_Fix32ToSigLibData (
  SLInt32_t * SIGLIB_PTR_DECL pSrc,
  SLData_t * SIGLIB_PTR_DECL pDst,
  const SLArrayIndex_t ArrayLength)
{
  SLArrayIndex_t  i;
