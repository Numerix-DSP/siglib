...
int main(int argc, char** argv)
{
  ...
  if (argc != 3) {
    printf("Useage error: splitfile <makefile prototype> <object file "
           "extension>\n");
    return (1);
  }

  if ((fpInputFileNames = fopen("srcfiles.txt", "rb")) == NULL) {
    printf("The source file list file \"srcfiles.txt\" could not be opened for "
           "reading\n");
    fcloseall();
    return (1);
  }
  ...
  if ((fpMakeFileOutput = fopen("SplitFiles\\makefile", "wb")) == NULL) {
    ...
    printf("Please ensure that the directory exists within the current "
           "directory\n");
    fcloseall();
    return (1);
  }
  ...
  while ((InputChar = getc(fpMakeFileInput)) != 0x1c) {    // Read start from makefile prototype
    if (InputChar == EOF) {
      printf("The prototype makefile did not contain the place holder "
             "character - 0x1c\n");
      fcloseall();
      return (1);
    }
