...
{
  //  wxMessageBox (_T("Got here"), _T("LineGraph::LineGraph"), wxOK |
  ...
  VSizer->Add(HSizer0, 0, wxALIGN_CENTER | wxALL, 0);    // Graph + left labels
  wxBoxSizer* HSizer1 = new wxBoxSizer(wxHORIZONTAL);
  VSizer->Add(HSizer1, 0, wxALIGN_CENTER | wxALL, 0);    // Bottom labels

  wxBoxSizer* VSizer0 = new wxBoxSizer(wxVERTICAL);
  HSizer0->Add(VSizer0, 0, wxALIGN_CENTER | wxALL, 0);    // Left labels
  wxBoxSizer* VSizer1 = new wxBoxSizer(wxVERTICAL);
  HSizer0->Add(VSizer1, 0, wxALIGN_CENTER | wxALL, 0);    // Graph
  ...
  HSizer1->SetMinSize(graphSize.GetWidth() + 80,
                      -1);    // Set min width for horizontal text
...
{
  //  wxMessageBox (_T("Got here"), _T("LineGraph::Draw"), wxOK |
  //  wxICON_EXCLAMATION, NULL);
...
#ifdef __WXMSW__
  GraphPaintDC.DrawRectangle(r.GetLeft() + 1, r.GetTop() + 1, r.GetRight() + 2,
                             r.GetBottom() + 2);    // X, Y, W, H
#else
  GraphPaintDC.DrawRectangle(r.GetLeft() + 2, r.GetTop() + 2, r.GetRight() + 1,
                             r.GetBottom() + 1);    // X, Y, W, H
#endif
...
#define VERT_TOTAL_NUMBER_OF_MARKS (VERT_NUMBER_OF_DIVISIONS * VERT_MARKS_PER_DIVISION)
  ...
  for (i = 1; i < VERT_TOTAL_NUMBER_OF_MARKS; i++) {
    ...
    if ((i & 3) == 0) {
      GraphPaintDC.DrawLine(0, (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS), Width + 1, (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS));
    }

    // Major mark - only on odd indices
    else if ((i & 1) == 0) {
      // Left hand side
      GraphPaintDC.DrawLine(0, (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS), MAJOR_MARK_SIZE, (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS));
      // Right hand side
      GraphPaintDC.DrawLine(Width - MAJOR_MARK_SIZE, (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS), Width,
                            (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS));
    }
    // Minor mark
    else {
      // Left hand side
      GraphPaintDC.DrawLine(0, (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS), MINOR_MARK_SIZE, (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS));
      // Right hand side
      GraphPaintDC.DrawLine(Width - MINOR_MARK_SIZE, (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS), Width,
                            (int)((i * Height) / VERT_TOTAL_NUMBER_OF_MARKS));
    }
  }
  //  wxMessageBox (_T("Got here"), _T("LineGraph::Draw"), wxOK |
  ...
  for (i = 1; i < 40; i++) {
    ...
    // Major mark - only on odd indices
      ...
      // Middle
      GraphPaintDC.DrawLine((int)((i * Width) / 40), (Height >> 1) - (MAJOR_MARK_SIZE - 1), (int)((i * Width) / 40),
                            (Height >> 1) + MAJOR_MARK_SIZE);
      // Bottom
    ...
    // Minor mark
      ...
      // Middle
      GraphPaintDC.DrawLine((int)((i * Width) / 40), (Height >> 1) - (MINOR_MARK_SIZE - 1), (int)((i * Width) / 40),
                            (Height >> 1) + MINOR_MARK_SIZE);
      // Bottom
  ...
  if (DataSetSize)    // Only plot the graph if the data set exists
  {
    ...
    for (int x = 0; x < DataSetSize; x++) {
      ...
      GraphOffset = ((double)(GraphSize.GetHeight() >> 1)) - ((((double)(GraphSize.GetHeight() >> 1)) / Max) * pDataSet[x]);
...
{
  //  wxMessageBox (_T("Got here"), _T("LineGraph::AddDataSet"), wxOK |
  //  wxICON_EXCLAMATION, NULL);
...
LineGraphCanvas::LineGraphCanvas(LineGraph* pLineGraph, wxPanel* parent, wxPoint GraphPoint, wxSize graphSize)
    :    // LineGraphCanvas::LineGraphCanvas (wxPanel *parent, wxPoint GraphPoint,
         // wxSize graphSize):
    wxWindow(parent, wxID_ANY, GraphPoint, graphSize)
{
  //  wxMessageBox (_T("Got here"), _T("LineGraphCanvas::LineGraphCanvas"), wxOK
  //  | wxICON_EXCLAMATION, NULL);

  GraphSize = graphSize;
  SetSize(GraphSize);    // Set the size of the canvas because it is not done by
                         // parent unless it is a frame
  m_owner = parent;
...
{
  //  wxMessageBox (_T("Got here"), _T("LineGraphCanvas::OnPaint"), wxOK |
  //  wxICON_EXCLAMATION, NULL);
...
{
  //  wxMessageBox (_T("Got here"), _T("LineGraphCanvas::OnMouseMove"), wxOK |
  //  wxICON_EXCLAMATION, NULL);

  wxString String;
  //  int         ArrayIndex = (int)(((double)event.GetX()) *
  //  m_LineGraph->GetStatusBarIndexMultiplier());
  ...
  if (m_LineGraph->GetQuantizeXAxisFlag() == true) {
    String.sprintf("%s: %1.3lf, Value: %1.3lf          ", m_LineGraph->GetStatusBarValueText().GetData(), (double)ArrayIndex,
                   m_LineGraph->GetValue(ArrayIndex));
  } else {
    String.sprintf("%s: %1.3lf, Value: %1.3lf          ", m_LineGraph->GetStatusBarValueText().GetData(),
...
PoleZeroPlot::PoleZeroPlot(wxWindow* parent, wxWindowID id, wxSize plotSize)
    : wxPanel(parent, id)
{
  //  wxMessageBox (_T("Start"), _T("PoleZeroPlot::PoleZeroPlot"), wxOK |
  //  wxICON_EXCLAMATION, NULL);
...
void PoleZeroPlot::Draw(wxDC& GraphPaintDC, bool WXUNUSED(draw_bitmaps))
{
  ...
  // wxMessageBox (_T("Got here - Start"), _T("PoleZeroPlot::Draw"), wxOK |
  // wxICON_EXCLAMATION, NULL);
  ...
  GraphPaintDC.DrawLine(XCenter - (Radius + 5), YCenter, XCenter + Radius + 5,
                        YCenter);    // Horiz
  GraphPaintDC.DrawLine(XCenter, YCenter - (Radius + 5), XCenter,
                        YCenter + Radius + 5);    // Virt
  ...
  if (DataSetSize)    // Only plot the graph if the data set exists
  {
    ...
    case PZ_COMPLEX_POLE_ZERO:
      for (int i = 0; i < DataSetSize; i++) {
        ...
        // Plot zeros - Above
        GraphPaintDC.DrawEllipse(XCenter + ((int)(XPos * ((double)Radius))) - 5, YCenter - ((int)(YPos * ((double)Radius))) - 5, 10, 10);
      }
      break;
...
{
  // wxMessageBox (_T("Got here: Start"), _T("PoleZeroPlot::AddDataSet"), wxOK |
  // wxICON_EXCLAMATION, NULL);
  ...
  // wxMessageBox (_T("Got here: End"), _T("PoleZeroPlot::AddDataSet"), wxOK |
  // wxICON_EXCLAMATION, NULL);
...
{
  PlotSize = plotSize;
  SetSize(PlotSize);    // Set the size of the canvas because it is not done by
                        // parent unless it is a frame
  m_owner = parent;
...
{
  //  wxMessageBox (_T("Got here"), _T("PoleZeroPlotCanvas::OnPaint"), wxOK |
  //  wxICON_EXCLAMATION, NULL);
