...
{
  //  wxMessageBox ("Got here", "GraphDisplay::GraphDisplay", wxOK |
  ...
  VSizer->Add(HSizer2p, 0, wxALIGN_CENTER | wxALL, 0);

  //  GraphPanel = new wxPanel (this, wxID_ANY, wxDefaultPosition,
  //  wxDefaultSize);
  ...
  // drawn
  //  DataGraph = new LineGraph (GraphPanel, wxID_ANY, wxSize (GRAPH_WIDTH,
  //  GRAPH_HEIGHT));
  ...
  //  GraphPanel->Show (TRUE);                // Hide the panel because we do
  //  not have any graphs showing

  // A combo box for getting required data type
  HSizer2->Add(new wxStaticText(this, -1, "Graph type: "), 0, wxALIGN_CENTER | wxALL, 0);
  static const wxString GraphChoices[] = {_T ("Sine"), _T ("Cosine"), _T ("Pole-zero")};
  GraphTypeComboBox = new wxComboBox(this, ID_DFGRAPH_GRAPH_TYPE_CHANGED, "Sine", wxDefaultPosition, wxSize(160, -1), WXSIZEOF(GraphChoices),
  ...
  //  sprintf (DebugString, "Height = %d, Width = %d", DebugSize.GetHeight(),
  //  DebugSize.GetWidth()); wxMessageBox (DebugString,
  //  _T("GraphDisplay::GraphDisplay"), wxOK | wxICON_EXCLAMATION, NULL);
...
{
  //  wxMessageBox (_T("Got here"), _T("GraphDisplay::OnGraphTypeChanged"), wxOK
  //  | wxICON_EXCLAMATION, NULL);
  ...
  //  wxMessageBox (_T(string), _T("LineGraphCanvas::LineGraphCanvas"), wxOK |
  //  wxICON_EXCLAMATION, NULL);
  ...
  if (GraphType == GRAPH_SINE) {
    ...
    //      GraphPanel->Show (TRUE);                // Unhide the panel because
    //      we now have graphs showing
  }
    ...
    pPolesAndZeros[0].real = 0.900000;
    pPolesAndZeros[0].imag = 0.000000;    // 0.9, > 0.0
    pPolesAndZeros[1].real = 0.984808;
    pPolesAndZeros[1].imag = 0.173648;    // 1.0, > 10.0
    pPolesAndZeros[2].real = 0.845723;
    pPolesAndZeros[2].imag = 0.307818;    // 0.9, > 20.0
    pPolesAndZeros[3].real = 0.866025;
    pPolesAndZeros[3].imag = 0.500000;    // 1.0, > 30.0
    pPolesAndZeros[4].real = 0.689440;
    pPolesAndZeros[4].imag = 0.578509;    // 0.9, > 40.0
    pPolesAndZeros[5].real = 0.642788;
    pPolesAndZeros[5].imag = 0.766044;    // 1.0, > 50.0
    pPolesAndZeros[6].real = 0.450000;
    pPolesAndZeros[6].imag = 0.779423;    // 0.9, > 60.0
    pPolesAndZeros[7].real = 0.342020;
    pPolesAndZeros[7].imag = 0.939693;    // 1.0, > 70.0
    pPolesAndZeros[8].real = 0.156283;
    pPolesAndZeros[8].imag = 0.886327;    // 0.9, > 80.0
    pPolesAndZeros[9].real = 0.000000;
    pPolesAndZeros[9].imag = 1.000000;    // 1.0, > 90.0

    PoleZeroDiagram->AddDataSet(pPolesAndZeros, 5, PZ_CONJUGATE_POLE_ZERO);
    //      PoleZeroDiagram->AddDataSet (pPolesAndZeros, 5,
    //      PZ_COMPLEX_POLE_ZERO); PoleZeroDiagram->AddDataSet (pPolesAndZeros,
    //      5, PZ_COMPLEX_ZERO); PoleZeroDiagram->AddDataSet (pPolesAndZeros, 5,
    //      PZ_CONJUGATE_ZERO); PoleZeroDiagram->AddDataSet (pPolesAndZeros, 5,
    //      PZ_COMPLEX_POLE); PoleZeroDiagram->AddDataSet (pPolesAndZeros, 5,
    //      PZ_CONJUGATE_POLE);

    // wxMessageBox (_T("Got here - PoleZeroDiagram->AddDataSet()"),
    // _T("GraphDisplay::OnGraphTypeChanged"), wxOK | wxICON_EXCLAMATION, NULL);
    DataGraph->Show(FALSE);
    // wxMessageBox (_T("Got here - DataGraph->Show (FALSE)"),
    // _T("GraphDisplay::OnGraphTypeChanged"), wxOK | wxICON_EXCLAMATION, NULL);
    PoleZeroDiagram->Show(TRUE);
    // wxMessageBox (_T("Got here - PoleZeroDiagram->Show (TRUE)"),
    // _T("GraphDisplay::OnGraphTypeChanged"), wxOK | wxICON_EXCLAMATION, NULL);
  ...
  // wxMessageBox (_T("Got here - Show (FALSE)"),
  // _T("GraphDisplay::OnGraphTypeChanged"), wxOK | wxICON_EXCLAMATION, NULL);
  // Show (TRUE);
  // wxMessageBox (_T("Got here - Show (TRUE)"),
  // _T("GraphDisplay::OnGraphTypeChanged"), wxOK | wxICON_EXCLAMATION, NULL);

  // wxMessageBox (_T("Got here - End"), _T("GraphDisplay::OnGraphTypeChanged"),
