...
void SIGLIB_FUNC_DECL SDA_CovarianceMatrix (
  const SLData_t * SIGLIB_PTR_DECL pSrc,
  SLData_t * SIGLIB_PTR_DECL pMeans,
  SLData_t * SIGLIB_PTR_DECL covarianceMatrix,
  const SLData_t inverseLength,
  const SLData_t finalDivisor,
  const SLArrayIndex_t numberOfDatasets,
  const SLArrayIndex_t ArrayLengths)
{
  ...
  if (((SLUFixData_t) ArrayLength & 0x1U) == 0U) {                  // Return mean of two middle values
    returnValue =
      ((*(pWorkArray + ((SLUFixData_t) ArrayLength >> 1U))) +
       (*(pWorkArray + (SLArrayIndex_t) ((SLUFixData_t) ArrayLength >> 1U) - 1))) / SIGLIB_TWO;
  }
  else {
