...
SLArrayIndex_t SIGLIB_FUNC_DECL SUF_AsyncCharacterLength(const SLArrayIndex_t NumberOfDataBits, const enum SLParity_t ParityType,
                                                         const SLArrayIndex_t NumberOfStopBits)
{
...
SLArrayIndex_t SIGLIB_FUNC_DECL SDA_SyncToAsyncConverter(const SLUInt8_t* pSrc, SLUInt8_t* pDst, const SLArrayIndex_t NumberOfDataBits,
                                                         const enum SLParity_t ParityType, const SLArrayIndex_t NumberOfStopBits,
                                                         const SLArrayIndex_t ArrayLength)
{
...
SLArrayIndex_t SIGLIB_FUNC_DECL SDA_AsyncToSyncConverter(const SLUInt8_t* pSrc, SLUInt8_t* pDst, const SLArrayIndex_t NumberOfDataBits,
                                                         const enum SLParity_t ParityType, SLArrayIndex_t* pParityErrorFlag,
                                                         const SLArrayIndex_t ArrayLength)
{
  SLUFixData_t DstBitIndex = SIGLIB_UFIX_ZERO;
...
void SIGLIB_FUNC_DECL SIF_AsyncAddRemoveStopBits(SLArrayIndex_t* pRemoveCount)
{
...
SLArrayIndex_t SIGLIB_FUNC_DECL SDA_AsyncRemoveStopBits(const SLUInt8_t* pSrc, SLUInt8_t* pDst, const SLArrayIndex_t NumberOfDataBits,
                                                        const enum SLParity_t ParityType, const SLArrayIndex_t RemoveRatio,
                                                        SLArrayIndex_t* pRemoveCount, const SLArrayIndex_t ArrayLength)
{
  SLUFixData_t DstBitIndex = SIGLIB_UFIX_ZERO;
  SLArrayIndex_t StartBitFoundFlag = SIGLIB_AI_ZERO;
  SLUFixData_t OutputWordCount = SIGLIB_UFIX_ZERO;
    ...
    for (SLUFixData_t j = 0; j < (SLArrayIndex_t)SIGLIB_BYTE_LENGTH; j++) {    // For each bit in the input byte
      if (StartBitFoundFlag == SIGLIB_AI_ZERO) {                               // Look for start bit == 0*/
        ...
        if (((SLUFixData_t)*pSrc) & ((SIGLIB_UFIX_ONE) << j)) {
          ...
          if (*pRemoveCount < RemoveRatio) {    // If we have not met the ratio
                                                // then copy the stop bit
                                                // Write output stop bit
            *pDst = (SLUInt8_t)(((SLArrayIndex_t)*pDst) + ((SIGLIB_UFIX_ONE) << DstBitIndex));

            DstBitIndex++;                                              // Increment output bit index
            if (DstBitIndex == (SLArrayIndex_t)SIGLIB_BYTE_LENGTH) {    // We have written a complete byte to
                                                                        // the output
              DstBitIndex = SIGLIB_AI_ZERO;
              *++pDst = (SLUInt8_t)0;    // Increment pointer and set new byte to '0'
        ...
        DstBitIndex++;                                              // Increment output bit index
        if (DstBitIndex == (SLArrayIndex_t)SIGLIB_BYTE_LENGTH) {    // We have written a complete
                                                                    // byte to the output
          DstBitIndex = SIGLIB_AI_ZERO;
          *++pDst = (SLUInt8_t)0;    // Increment pointer and set new byte to '0'
          OutputWordCount++;         // Increment number of output characters
...
SLArrayIndex_t SIGLIB_FUNC_DECL SDA_AsyncAddStopBits(const SLUInt8_t* pSrc, SLUInt8_t* pDst, const SLArrayIndex_t NumberOfDataBits,
                                                     const enum SLParity_t ParityType, const SLArrayIndex_t AddRatio, SLArrayIndex_t* pAddCount,
                                                     const SLArrayIndex_t ArrayLength)
{
  SLUFixData_t DstBitIndex = SIGLIB_UFIX_ZERO;
  SLArrayIndex_t StartBitFoundFlag = SIGLIB_AI_ZERO;
      ...
      if (StartBitFoundFlag == SIGLIB_AI_ZERO) {                               // Look for start bit == 0*/
        ...
        if (((SLUFixData_t)*pSrc) & ((SIGLIB_UFIX_ONE) << j)) {
          ...
          *pDst = (SLUInt8_t)(((SLArrayIndex_t)*pDst) + ((SIGLIB_UFIX_ONE) << DstBitIndex));
          ...
          if (StopBitFoundFlag == SIGLIB_AI_ZERO) {    // Account for when stop bits have been removed
                                                       // completely
            (*pAddCount)++;                            // Increment add counter
            if (*pAddCount >= AddRatio) {              // If we have met the ratio then add
                                                       // the new stop bit
              *pAddCount = SIGLIB_AI_ZERO;
              ...
              *pDst = (SLUInt8_t)(((SLArrayIndex_t)*pDst) + ((SIGLIB_UFIX_ONE) << DstBitIndex));
              ...
              DstBitIndex++;                                              // Increment output bit index
              if (DstBitIndex == (SLArrayIndex_t)SIGLIB_BYTE_LENGTH) {    // We have written a complete byte
                                                                          // to the output
                DstBitIndex = SIGLIB_AI_ZERO;
                *++pDst = (SLUInt8_t)0;    // Increment pointer and set new byte to '0'
                OutputWordCount++;         // Increment number of output characters
...
SLArrayIndex_t SIGLIB_FUNC_DECL SDA_DecreaseWordLength(const SLUInt8_t* pSrc, SLUInt8_t* pDst, const SLArrayIndex_t InputWordLength,
                                                       const SLArrayIndex_t OutputWordLength, const SLArrayIndex_t ArrayLength)
{
...
SLArrayIndex_t SIGLIB_FUNC_DECL SDA_IncreaseWordLength(const SLUInt8_t* pSrc, SLUInt8_t* pDst, const SLArrayIndex_t InputWordLength,
                                                       const SLArrayIndex_t OutputWordLength, const SLArrayIndex_t ArrayLength)
{
  SLUFixData_t InputStartIndex = SIGLIB_UFIX_ZERO;
  SLUFixData_t OutputStartIndex = SIGLIB_UFIX_ZERO;
