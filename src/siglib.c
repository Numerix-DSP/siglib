...
#  define MAX_SPLIT_LENGTH 4               // MAX_SPLIT_LENGTH number of lines printed at start and end
  ...
  if (arrayLength < MAX_INCLUSIVE_ARRAY_LENGTH) {
    for (SLArrayIndex_t i = 0; i < arrayLength; i++) {
      if ((i % NUMBER_OF_SAMPLES_PER_LINE) == 0) {
...
/********************************************************
 * Function: SUF_PrintComplexMatrix
 *
 * Parameters:
 *  const SLData_t *pSrcReal,      - Pointer to real data array
 *  const SLData_t *pSrcImag,      - Pointer to imag data array
 ...
 *  void
 *
 * Description: Prints a complex matrix of data to the console.
 *
 ********************************************************/

void SIGLIB_FUNC_DECL SUF_PrintComplexMatrix(const SLData_t* pSrcReal, const SLData_t* pSrcImag, const SLArrayIndex_t nRows,
                                             const SLArrayIndex_t nCols)
{
  for (SLArrayIndex_t i = 0; i < nRows; i++) {
    for (SLArrayIndex_t j = 0; j < nCols; j++) {
      SLData_t real = *(pSrcReal + (i * nCols) + j);
      SLData_t imag = *(pSrcImag + (i * nCols) + j);
      if (real >= SIGLIB_ZERO) {
        SUF_Printf(" ");
      }
      if (imag >= SIGLIB_ZERO) {
        //          SUF_Printf ("%le + j%le  ", real, imag);
        SUF_Printf("%1.4lf +j%1.4lf  ", real, imag);
      } else {
        //          SUF_Printf ("%le - j%le  ", real, -imag);
        SUF_Printf("%1.4lf -j%1.4lf  ", real, -imag);
      }
  ...
  SUF_Printf("\n");
}    // End of SUF_PrintComplexMatrix()

/********************************************************
 * Function: SUF_PrintComplexNumber
 *
 * Parameters:
 *  const SLData_t *pSrcReal,      - Pointer to real data array
 *  const SLData_t *pSrcImag,      - Pointer to imag data array
...
void SIGLIB_FUNC_DECL SUF_PrintComplexNumber(const SLData_t real, const SLData_t imag)
{
  if (real >= SIGLIB_ZERO) {
    SUF_Printf(" ");
  }
