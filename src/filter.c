...
// Interpolate coefficient
      Tmp =
        ((((SDS_Sin (SIGLIB_PI * (((SLData_t) i) - SIGLIB_ONE) / SymbolPeriod) / (SIGLIB_PI * (((SLData_t) i) - SIGLIB_ONE) / SymbolPeriod)) *
           SDS_Cos (Alpha * SIGLIB_PI * (((SLData_t) i) - SIGLIB_ONE) / SymbolPeriod)) / (SIGLIB_ONE -
                                                                                          (SIGLIB_FOUR * Alpha * Alpha *
                                                                                           ((((SLData_t) i) -
                                                                                             SIGLIB_ONE) / SymbolPeriod) * ((((SLData_t) i) -
                                                                                                                             SIGLIB_ONE) /
                                                                                                                            SymbolPeriod)))) +
         (((SDS_Sin (SIGLIB_PI * (((SLData_t) i) + SIGLIB_ONE) / SymbolPeriod) / (SIGLIB_PI * (((SLData_t) i) + SIGLIB_ONE) / SymbolPeriod)) *
           SDS_Cos (Alpha * SIGLIB_PI * (((SLData_t) i) + SIGLIB_ONE) / SymbolPeriod)) / (SIGLIB_ONE -
      ...
      Tmp =
        ((SDS_Sin (SIGLIB_PI * ((SLData_t) i) / SymbolPeriod) / (SIGLIB_PI * ((SLData_t) i) / SymbolPeriod)) *
         SDS_Cos (Alpha * SIGLIB_PI * ((SLData_t) i) / SymbolPeriod)) / (SIGLIB_ONE -
                                                                         (SIGLIB_FOUR * Alpha * Alpha * (((SLData_t) i) / SymbolPeriod) *
                                                                          (((SLData_t) i) / SymbolPeriod)));
    }
      ...
      Tmp = ((((SIGLIB_FOUR * Alpha) / (SIGLIB_PI * SDS_Sqrt (SymbolPeriod))) *
              ((SDS_Cos ((SIGLIB_ONE + Alpha) * SIGLIB_PI * (((SLData_t) i) - SIGLIB_ONE) / SymbolPeriod) +
                (SDS_Sin ((SIGLIB_ONE - Alpha) * SIGLIB_PI * (((SLData_t) i) - SIGLIB_ONE) / SymbolPeriod) /
                 (SIGLIB_FOUR * Alpha * (((SLData_t) i) - SIGLIB_ONE) / SymbolPeriod))) / (SIGLIB_ONE -
                                                                                           (SIGLIB_SIXTEEN * Alpha * Alpha *
                                                                                            ((((SLData_t) i) -
                                                                                              SIGLIB_ONE) / SymbolPeriod) * ((((SLData_t) i) -
                                                                                                                              SIGLIB_ONE) /
                                                                                                                             SymbolPeriod))))) +
...
// Update new Kalman gain (K)
  ...
  SMX_Multiply ((SLData_t *) kf->H, (SLData_t *) kf->P, K, 2, 2, 2);  // Use K as temporary storage
...
// Update Process estimate error estimation (P)
  SLData_t        KH[4];                                            // [2][2]
  SLData_t        identity_2x2[4] = { SIGLIB_ONE, SIGLIB_ZERO, SIGLIB_ZERO, SIGLIB_ONE }; // [2][2] = { { SIGLIB_ONE, SIGLIB_ZERO }, { SIGLIB_ZERO, SIGLIB_ONE } };
  SLData_t        KHP[4];                                           // [2][2]
