...
SLData_t SIGLIB_FUNC_DECL SDA_ExtractOrder(const SLData_t* SIGLIB_PTR_DECL pSrc, const SLArrayIndex_t OrderToExtract,
                                           const SLArrayIndex_t NumberOfAdjacentSamples, const SLData_t FirstOrdFrq, const SLArrayIndex_t FFTLen,
                                           const SLData_t SamplePeriod, const SLArrayIndex_t ArrayLength)
{
...
SLData_t SIGLIB_FUNC_DECL SDA_SumLevel(const SLData_t* SIGLIB_PTR_DECL pSrc, const enum SLSignalCoherenceType_t SignalSourceType,
                                       const SLArrayIndex_t LogMagnitudeFlag, const SLArrayIndex_t ArrayLength)
{
...
SLData_t SIGLIB_FUNC_DECL SDA_SumLevelWholeSpectrum(const SLData_t* SIGLIB_PTR_DECL pSrc, const enum SLSignalCoherenceType_t SignalSourceType,
                                                    const SLArrayIndex_t LogMagnitudeFlag, const SLData_t LinearScale,
                                                    const SLArrayIndex_t ArrayLength)
{
  SLData_t SumLevel = SIGLIB_ZERO;
...
void SIGLIB_FUNC_DECL SIF_OrderAnalysis(SLData_t* SIGLIB_PTR_DECL pSincLUT, SLData_t* pLookUpTablePhaseGain,
                                        const SLArrayIndex_t NumberOfAdjacentSamples, const SLArrayIndex_t SincLutLength,
                                        SLData_t* SIGLIB_PTR_DECL pWindowCoeffs, const enum SLWindow_t WindowType, const SLData_t WindowCoefficient,
                                        SLData_t* pWindowInverseCoherentGain, SLData_t* SIGLIB_PTR_DECL pFFTCoeffs,
                                        SLArrayIndex_t* SIGLIB_PTR_DECL pBitRevAddrTable, SLData_t* SIGLIB_PTR_DECL pRealAverage,
                                        SLData_t* SIGLIB_PTR_DECL pImagAverage, const SLArrayIndex_t FFTLength)
{
  ...
  *pWindowInverseCoherentGain = SDA_WindowInverseCoherentGain(pWindowCoeffs,    // Source array pointer
  ...
  SIF_Fft(pFFTCoeffs,          // Pointer to FFT coefficients
          pBitRevAddrTable,    // Bit reverse mode flag / Pointer to bit reverse
                               // address table
...
SLData_t SIGLIB_FUNC_DECL SDA_OrderAnalysis(
    const SLData_t* SIGLIB_PTR_DECL pSrc, SLData_t* SIGLIB_PTR_DECL pLocalProcArray, SLData_t* SIGLIB_PTR_DECL pDst,
    const SLData_t* SIGLIB_PTR_DECL pSincLUT, const SLData_t pLookUpTablePhaseGain, const SLData_t FirstOrderFrequency, const SLData_t Speed,
    const SLArrayIndex_t SincNumberOfAdjacentSamples, SLData_t* SIGLIB_PTR_DECL pWindowCoeffs, const SLData_t WindowInverseCoherentGain,
    SLData_t* SIGLIB_PTR_DECL pFFTCoeffs, SLArrayIndex_t* SIGLIB_PTR_DECL pBitRevAddrTable, SLData_t* SIGLIB_PTR_DECL pRealAverage,
    SLData_t* SIGLIB_PTR_DECL pImagAverage, const SLArrayIndex_t LogMagnitudeFlag, SLData_t* SIGLIB_PTR_DECL pOrderArray,
    const SLArrayIndex_t BaseOrder, const SLArrayIndex_t NumberOfOrders, const SLArrayIndex_t OrderNumberOfAdjacentSamples,
    const SLData_t SamplePeriod, const enum SLSignalCoherenceType_t SignalSourceType, const SLData_t dBScale,
    const SLArrayIndex_t NumberOfOrdersToSum, const SLArrayIndex_t SourceLength, const SLArrayIndex_t FFTLength, const SLArrayIndex_t Log2FFTLength)
{
  SLArrayIndex_t ResultLength = (SLArrayIndex_t)((SLUFixData_t)FFTLength >> 1U);    // Calculate the length of the output result
...
// Resample only as many samples as we require
  ...
  ResampleResultLength =
      ...
      SDA_ResampleSincNSamples(pSrc,                           // Pointer to source array
                               ...
                               SincNumberOfAdjacentSamples,    // Number of adjacent samples for
                                                               // interpolation
...
//  SUF_Debugfprintf ("Extracting orders\n");
  ...
  for (SLArrayIndex_t i = 0; i < NumberOfOrders; i++) {                       // Extract the required orders from the results
    *(pOrderArray + i) = SDA_ExtractOrder(pDst,                               // Pointer to source array
                                          ...
                                          OrderNumberOfAdjacentSamples,       // Number of samples to search either side
                                                                              // of centre
...
//  SUF_DebugPrintArray (pOrderArray, NumberOfOrders);
  ...
  OrderSum = SDA_SumLevel(pOrderArray,             // Pointer to source array
