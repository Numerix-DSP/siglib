...
SLError_t SIGLIB_FUNC_DECL SDA_SignalGenerate(SLData_t* SIGLIB_PTR_DECL pDst, const enum SLSignal_t SLSignal_t, const SLData_t Peak,
                                              const enum SLSignalFillMode_t FillMode, SLData_t Frequency, const SLData_t Offset,
                                              const SLData_t Param, const SLData_t EndValue, SLData_t* SIGLIB_PTR_DECL PhaseOffset,
                                              SLData_t* SIGLIB_PTR_DECL CurrentValue, const SLArrayIndex_t arrayLength)
{
...
#  endif
  ...
  if (FillMode == SIGLIB_FILL) {    // Generate signal and fill array
    ...
    case SIGLIB_GAUSSIAN_NOISE:    // Gaussian (normal) distribution using the
                                   // Box-Muller Method
      for (SLArrayIndex_t i = 0; i < arrayLength; i++) {
        if (!*PhaseOffset) {
          ...
          if (SquareSum != SIGLIB_ZERO) {
            ...
            // Gaussian sample = Param * SampleN * sqrt(SIGLIB_MINUS_TWO *
            // ln(SquareSum)/SquareSum
...
SLError_t SIGLIB_FUNC_DECL SDS_SignalGenerate(SLData_t* SIGLIB_PTR_DECL pDst, const enum SLSignal_t SLSignal_t, const SLData_t Peak,
                                              const enum SLSignalFillMode_t FillMode, SLData_t Frequency, const SLData_t Offset,
                                              const SLData_t Param, const SLData_t EndValue, SLData_t* SIGLIB_PTR_DECL PhaseOffset,
...
void SIGLIB_FUNC_DECL SIF_Resonator(SLData_t* SIGLIB_PTR_DECL pState, const SLData_t Frequency, SLData_t* pCosCoeff, SLData_t* pSinCoeff)
{
...
void SIGLIB_FUNC_DECL SDA_Resonator(const SLData_t* SIGLIB_PTR_DECL pSrc, SLData_t* SIGLIB_PTR_DECL pDst, SLData_t* SIGLIB_PTR_DECL pDelay,
                                    const SLData_t CosCoeff, const SLData_t SinCoeff, const SLArrayIndex_t arrayLength)
{
#if (SIGLIB_ARRAYS_ALIGNED)
#  ifdef __TMS320C6X__             // Defined by TI compiler
...
#  endif
  ...
  for (SLArrayIndex_t i = 0; i < arrayLength; i++) {
    ...
    // SUF_Debugfprintf ("%d: Sum = %3.2lf, Delay0 = %3.2lf, Delay1 = %3.2lf,
    // Delay2 = %3.2lf\n", (int)i, Sum*16383.0, Delay0*16383.0, Delay1*16383.0,
    // Delay2*16383.0);
...
void SIGLIB_FUNC_DECL SIF_Resonator1(SLData_t* SIGLIB_PTR_DECL pState, const SLData_t Frequency, SLData_t* pCosCoeff, SLData_t* pSinCoeff,
...
void SIGLIB_FUNC_DECL SDA_Resonator1(SLData_t* SIGLIB_PTR_DECL pDst, const SLData_t Magn, SLData_t* SIGLIB_PTR_DECL pDelay,
                                     SLFixData_t* FirstTimeFlag, const SLData_t CosCoeff, const SLData_t SinCoeff, const SLArrayIndex_t arrayLength)
{
#if (SIGLIB_ARRAYS_ALIGNED)
#  ifdef __TMS320C6X__             // Defined by TI compiler
  _nassert((int)pDst % 8 == 0);    // Align arrays on 64 bit double word boundary for LDDW
...
void SIGLIB_FUNC_DECL SDA_Resonator1Add(SLData_t* SIGLIB_PTR_DECL pDst, const SLData_t Magn, SLData_t* SIGLIB_PTR_DECL pDelay,
                                        SLFixData_t* FirstTimeFlag, const SLData_t CosCoeff, const SLData_t SinCoeff,
                                        const SLArrayIndex_t arrayLength)
{
#if (SIGLIB_ARRAYS_ALIGNED)
#  ifdef __TMS320C6X__             // Defined by TI compiler
...
void SIGLIB_FUNC_DECL SDA_SignalGeneratePolarWhiteNoise(SLComplexRect_s* SIGLIB_PTR_DECL pDst, const SLData_t NoisePeak,
                                                        const enum SLSignalFillMode_t FillMode, const SLArrayIndex_t arrayLength)
{
...
void SIGLIB_FUNC_DECL SDA_SignalGeneratePolarGaussianNoise(SLComplexRect_s* SIGLIB_PTR_DECL pDst, const SLData_t NoiseVariance,
                                                           SLData_t* NoisePhaseOffset, SLData_t* NoiseCurrentValue,
                                                           const enum SLSignalFillMode_t FillMode, const SLArrayIndex_t arrayLength)
{
  SLComplexRect_s NoiseRectLocal;
  SLComplexPolar_s NoisePolarLocal;
    ...
    for (SLArrayIndex_t i = 0; i < arrayLength; i++) {
      if (!*NoisePhaseOffset) {
        ...
        if (SquareSum != SIGLIB_ZERO) {
          ...
          // Gaussian sample = NoiseVariance * SampleN * sqrt(SIGLIB_MINUS_TWO *
          // ln(SquareSum)/SquareSum
          Sample1 *= GaussianBias;
          Sample2 *= GaussianBias;
        } else {
...
SLComplexRect_s SIGLIB_FUNC_DECL SDS_SignalGeneratePolarGaussianNoise(const SLData_t NoiseVariance, SLData_t* NoisePhaseOffset,
                                                                      SLData_t* NoiseCurrentValue)
{
  SLComplexRect_s NoiseRectLocal;
  SLComplexPolar_s NoisePolarLocal;
...
void SIGLIB_FUNC_DECL SDA_SignalAddPolarJitterAndGaussianNoise(const SLComplexRect_s* SIGLIB_PTR_DECL pSrc, SLComplexRect_s* SIGLIB_PTR_DECL pDst,
                                                               const SLData_t JitterFrequency, const SLData_t JitterMagnitude,
                                                               SLData_t* JitterPhaseOffset, const SLData_t NoiseVariance,
                                                               SLData_t* NoisePhaseOffset, SLData_t* NoiseCurrentValue,
                                                               const SLArrayIndex_t arrayLength)
{
  SLComplexRect_s NoiseRectLocal;
...
SLComplexRect_s SIGLIB_FUNC_DECL SDS_SignalAddPolarJitterAndGaussianNoise(const SLComplexRect_s Src, const SLData_t JitterFrequency,
                                                                          const SLData_t JitterMagnitude, SLData_t* JitterPhaseOffset,
                                                                          const SLData_t NoiseVariance, SLData_t* NoisePhaseOffset,
                                                                          SLData_t* NoiseCurrentValue)
{
  SLComplexRect_s NoiseRectLocal;
  SLComplexPolar_s NoisePolarLocal;
...
void SIGLIB_FUNC_DECL SDA_Ramp(SLData_t* pDst, const SLData_t startValue, const SLData_t incrementValue, const SLArrayIndex_t arrayLength)
{
...
SLData_t SIGLIB_FUNC_DECL SDS_RandomNumber(const SLData_t lowerBound, const SLData_t upperBound)
{
...
void SIGLIB_FUNC_DECL SDA_RandomNumber(SLData_t* SIGLIB_PTR_DECL pDst, const SLData_t lowerBound, const SLData_t upperBound,
                                       const SLArrayIndex_t arrayLength)
{
  for (SLArrayIndex_t i = 0; i < arrayLength; i++) {
