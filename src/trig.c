...
void SIGLIB_FUNC_DECL SIF_FastSin(SLData_t* pSineTable, const SLArrayIndex_t TableLength)
{
...
SLData_t SIGLIB_FUNC_DECL SDS_FastSin(const SLData_t* SIGLIB_PTR_DECL pSineTable, SLData_t* pSineTablePhase, const SLData_t SineFrequency,
                                      const SLArrayIndex_t SineTableLength)
{
...
void SIGLIB_FUNC_DECL SIF_FastCos(SLData_t* pCosineTable, const SLArrayIndex_t TableLength)
{
  SLData_t PhaseMultiplier = (SIGLIB_TWO_PI / ((SLData_t)TableLength));
...
SLData_t SIGLIB_FUNC_DECL SDS_FastCos(const SLData_t* SIGLIB_PTR_DECL pCosineTable, SLData_t* pCosineTablePhase, const SLData_t CosineFrequency,
                                      const SLArrayIndex_t CosineTableLength)
{
...
void SIGLIB_FUNC_DECL SIF_FastSinCos(SLData_t* pSineTable, const SLArrayIndex_t SinCosPeriod)
{
...
void SIGLIB_FUNC_DECL SDS_FastSinCos(const SLData_t* SIGLIB_PTR_DECL pSineTable, SLData_t* pSinDst, SLData_t* pCosDst, SLData_t* pSineTablePhase,
                                     const SLData_t SineFrequency, const SLArrayIndex_t SinCosPeriod)
{
  *pSinDst = pSineTable[(SLArrayIndex_t)((*pSineTablePhase * (SLData_t)SinCosPeriod) + 0.1)];
  *pCosDst = pSineTable[(SLArrayIndex_t)(((*pSineTablePhase * (SLData_t)SinCosPeriod)) + 0.1) + (SLArrayIndex_t)((SLUFixData_t)SinCosPeriod >> 2U)];
...
void SIGLIB_FUNC_DECL SIF_QuickSin(SLData_t* pSineTable, SLData_t* pPhaseGain, const SLArrayIndex_t TableLength)
{
...
SLData_t SIGLIB_FUNC_DECL SDS_QuickSin(const SLData_t Angle, const SLData_t* SIGLIB_PTR_DECL pSineTable, const SLData_t PhaseGain)
{
...
void SIGLIB_FUNC_DECL SIF_QuickCos(SLData_t* pCosineTable, SLData_t* pPhaseGain, const SLArrayIndex_t TableLength)
{
  *pPhaseGain = (SIGLIB_TWO_PI / TableLength);    // Calculate phase increment through lookup table
  for (SLArrayIndex_t i = 0; i < TableLength; i++) {
...
SLData_t SIGLIB_FUNC_DECL SDS_QuickCos(const SLData_t Angle, const SLData_t* SIGLIB_PTR_DECL pCosineTable, const SLData_t PhaseGain)
{
  SLData_t LocalAngle = Angle;
  if (LocalAngle >= SIGLIB_ZERO) {
    while (LocalAngle >= SIGLIB_TWO_PI) {    // Wrap into 0 -> 2.0*pi
...
void SIGLIB_FUNC_DECL SIF_QuickSinCos(SLData_t* pSineTable, SLData_t* pPhaseGain, const SLArrayIndex_t SinCosPeriod)
{
  SLArrayIndex_t TableLength = ((SinCosPeriod * 5) / 4);
  *pPhaseGain = (SIGLIB_TWO_PI / TableLength);    // Calculate phase increment through lookup table
  for (SLArrayIndex_t i = 0; i < TableLength; i++) {
...
void SIGLIB_FUNC_DECL SDS_QuickSinCos(const SLData_t Angle, const SLData_t* SIGLIB_PTR_DECL pSineTable, SLData_t* pSinDst, SLData_t* pCosDst,
                                      const SLData_t PhaseGain, const SLArrayIndex_t SinCosPeriod)
{
  SLData_t LocalAngle = Angle;
  if (LocalAngle >= SIGLIB_ZERO) {
    while (LocalAngle >= SIGLIB_TWO_PI) {    // Wrap into 0 -> 2.0*pi
...
void SIGLIB_FUNC_DECL SIF_QuickTan(SLData_t* pTanTable, SLData_t* pPhaseGain, const SLArrayIndex_t TableLength)
{
  *pPhaseGain = (SIGLIB_TWO_PI / TableLength);    // Calculate phase increment through lookup table
  for (SLArrayIndex_t i = 0; i < TableLength; i++) {
...
SLData_t SIGLIB_FUNC_DECL SDS_QuickTan(const SLData_t Angle, const SLData_t* SIGLIB_PTR_DECL pTanTable, const SLData_t PhaseGain)
{
  SLData_t LocalAngle = Angle;
  if (LocalAngle >= SIGLIB_ZERO) {
    while (LocalAngle >= SIGLIB_TWO_PI) {    // Wrap into 0 -> 2.0*pi
...
SLData_t SIGLIB_FUNC_DECL SDS_Sinc(const SLData_t x)
{
  if (x == SIGLIB_ZERO) {
    return SIGLIB_ONE;
  } else {
...
void SIGLIB_FUNC_DECL SIF_QuickSinc(SLData_t* pSincTable, SLData_t* pPhaseGain, const SLData_t MaxXValue, const SLArrayIndex_t TableLength)
{
  //    SLData_t localPhaseGain = ((MaxXValue + 1.) /
  //    ((SLData_t)(TableLength-1)));   // Calculate phase increment through
  //    lookup table
...
SLData_t SIGLIB_FUNC_DECL SDS_QuickSinc(const SLData_t x, const SLData_t* SIGLIB_PTR_DECL pSincTable, const SLData_t PhaseGain)
{
  if (x >= SIGLIB_ZERO) {
