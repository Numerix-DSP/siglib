...
// Implement early-late gate trigger real data paths
  ELGTriggerOutput =
    SDS_EarlyLateGateSquarePulse (RealTmp, pELGMatchedFilterState,
                                  ...
                                  pELGZeroCrossingPreviousSample, pELGTriggerCount, pELGTriggerDetectedFlag, pELGTriggerUpdatedFlag, SymbolLength);
...
// Delay complex data to match trigger output to account for filter delays
  SDS_FixedDelayComplex (RealTmp, ImagTmp, &RealDelayedOutput, &ImagDelayedOutput,
                         pELGRealOutputSynchDelay, pELGImagOutputSynchDelay, pELGOutputSynchDelayIndex, ELGOutputSynchDelayLength);
...
const SLComplexRect_s siglib_numerix_OPSKTxConstellation[] = {      // Hex  Bit reversed
...
const SLComplexRect_s siglib_numerix_OPSKRxConstellation[] = {      // Hex  Bit reversed
  { SIGLIB_SQRT_TWO_DIV_TWO, -SIGLIB_SQRT_TWO_DIV_TWO },            // 0x5 0b101
  { SIGLIB_ONE, SIGLIB_ZERO },                                      // 0x7 0b111
  { SIGLIB_MINUS_ONE, SIGLIB_ZERO },                                // 0x4 0b001
  { -SIGLIB_SQRT_TWO_DIV_TWO, SIGLIB_SQRT_TWO_DIV_TWO },            // 0x6 0b011
...
// Decode constellation - Find nearest constellation point
      for (int i = 0; i < SIGLIB_OPSK_NUMBER_OF_PHASES; i++) {
        DemodErrorArray[i] = SDS_EuclideanDistanceSquared (siglib_numerix_OPSKTxConstellation[i].real,
                                                           siglib_numerix_OPSKTxConstellation[i].imag, pMagn->real, pMagn->imag);
      }
  ...
  SLData_t * pSampleSum)
{
  SIF_CostasLoop (pCostasLpVCOPhase, pCostasLoopVCOLookUpTable, CostasLoopVCOLookUpTableSize,
                  ...
                  pCostasLpLPF2Index, pCostasLpLPFCoeffs, CostasLpLPFLength, pCostasLpLoopFilterState, pSample);
