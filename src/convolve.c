...
void SIGLIB_FUNC_DECL SDA_ConvolveLinear (const SLData_t * SIGLIB_PTR_DECL pSrc,
    const SLData_t * SIGLIB_PTR_DECL pImpulseResponse,
    SLData_t * SIGLIB_PTR_DECL pDst,
    const SLArrayIndex_t InputLength,
    const SLArrayIndex_t ImpulseLength)
...
#pragma DATA_ALIGN(pImpulseResponse, 8);    // Align arrays on 64 bit double word boundary for LDDW
...
void SIGLIB_FUNC_DECL SDA_ConvolvePartial (const SLData_t * SIGLIB_PTR_DECL pSrc,
    const SLData_t * SIGLIB_PTR_DECL pImpulseResponse,
    SLData_t * SIGLIB_PTR_DECL pDst,
    const SLArrayIndex_t InputLength,
    const SLArrayIndex_t ImpulseLength)
...
void SIGLIB_FUNC_DECL SDA_ConvolveCircular (const SLData_t * SIGLIB_PTR_DECL pSrc,
    const SLData_t * SIGLIB_PTR_DECL pImpulseResponse,
    SLData_t * SIGLIB_PTR_DECL pDst,
    const SLArrayIndex_t SampleLength)
...
void SIGLIB_FUNC_DECL SDA_ConvolveLinearComplex (const SLData_t * SIGLIB_PTR_DECL pSrcReal,
    const SLData_t * SIGLIB_PTR_DECL pSrcImag,
    const SLData_t * SIGLIB_PTR_DECL pImpulseResponseReal,
    const SLData_t * SIGLIB_PTR_DECL pImpulseResponseImag,
    SLData_t * SIGLIB_PTR_DECL pDstReal,
...

{
    ...
    *pDstImag++ = (*p_ShortestReal * *p_LongestImag) + (*p_ShortestImag * *p_LongestReal);
    ...
    for (i = 0; i < Diff; i++) {                        // Middle overlap stage
        ...
        for (j = 1; j < LenShortest; j++) {
            SumProdReal += (p_ShortestReal[LenShortest-1-j] * p_LongestReal[i+1+j]) - (p_ShortestImag[LenShortest-1-j] * p_LongestImag[i+1+j]);
            SumProdImag += (p_ShortestReal[LenShortest-1-j] * p_LongestImag[i+1+j]) + (p_ShortestImag[LenShortest-1-j] * p_LongestReal[i+1+j]);
        }
        *pDstReal++ = SumProdReal;
        *pDstImag++ = SumProdImag;
    }
    ...
    for (i = 0; i < (LenShortest - 1); i++) {           // Final overlap stage
        ...
        for (j = 1; j < (LenShortest - 1 - i); j++) {
            SumProdReal += (p_ShortestReal[LenShortest-1-j] * p_LongestReal[Diff+1+i+j]) - (p_ShortestImag[LenShortest-1-j] * p_LongestImag[Diff+1+i+j]);
            SumProdImag += (p_ShortestReal[LenShortest-1-j] * p_LongestImag[Diff+1+i+j]) + (p_ShortestImag[LenShortest-1-j] * p_LongestReal[Diff+1+i+j]);
        }
        *pDstReal++ = SumProdReal;
        *pDstImag++ = SumProdImag;
    }
...
void SIGLIB_FUNC_DECL SDA_ConvolvePartialComplex (const SLData_t * SIGLIB_PTR_DECL pSrcReal,
    const SLData_t * SIGLIB_PTR_DECL pSrcImag,
    const SLData_t * SIGLIB_PTR_DECL pImpulseResponseReal,
    const SLData_t * SIGLIB_PTR_DECL pImpulseResponseImag,
    SLData_t * SIGLIB_PTR_DECL pDstReal,
...
void SIGLIB_FUNC_DECL SDA_ConvolveCircularComplex (const SLData_t * SIGLIB_PTR_DECL pSrcReal,
    const SLData_t * SIGLIB_PTR_DECL pSrcImag,
    const SLData_t * SIGLIB_PTR_DECL pImpulseResponseReal,
    const SLData_t * SIGLIB_PTR_DECL pImpulseResponseImag,
    SLData_t * SIGLIB_PTR_DECL pDstReal,
...
void SIGLIB_FUNC_DECL SDA_FftDeconvolution (SLData_t * SIGLIB_PTR_DECL pSrcReal,
...
void SIGLIB_FUNC_DECL SIF_FftDeconvolutionPre (const SLData_t * SIGLIB_PTR_DECL pImpulseReal,
    SLData_t * SIGLIB_PTR_DECL pImpulseFdReal,
    SLData_t * SIGLIB_PTR_DECL pImpulseFdImag,
    const SLData_t Gamma,
    const SLData_t * SIGLIB_PTR_DECL pFFTCoeffs,
...
void SIGLIB_FUNC_DECL SDA_FftDeconvolutionPre (SLData_t * SIGLIB_PTR_DECL pSrcReal,
    SLData_t * SIGLIB_PTR_DECL pSrcImag,
    const SLData_t * SIGLIB_PTR_DECL pImpulseFdReal,
    const SLData_t * SIGLIB_PTR_DECL pImpulseFdImag,
    const SLData_t * SIGLIB_PTR_DECL pFFTCoeffs,
