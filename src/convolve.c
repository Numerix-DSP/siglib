                                                ...
                                                const SLArrayIndex_t ImpulseLength)
{
  ...
  *pDstImag++ = (*p_ShortestReal * *p_LongestImag) + (*p_ShortestImag * *p_LongestReal);
  ...
  for (SLArrayIndex_t i = 0; i < Diff; i++) {    // Middle overlap stage
    SLData_t SumProdReal = (p_ShortestReal[LenShortest - 1] * p_LongestReal[i + 1]) - (p_ShortestImag[LenShortest - 1] * p_LongestImag[i + 1]);
    SLData_t SumProdImag = (p_ShortestReal[LenShortest - 1] * p_LongestImag[i + 1]) + (p_ShortestImag[LenShortest - 1] * p_LongestReal[i + 1]);
    for (SLArrayIndex_t j = 1; j < LenShortest; j++) {
      SumProdReal +=
  ...
  for (SLArrayIndex_t i = 0; i < (LenShortest - 1); i++) {    // Final overlap stage
    ...
    for (SLArrayIndex_t j = 1; j < (LenShortest - 1 - i); j++) {
      SumProdReal += (p_ShortestReal[LenShortest - 1 - j] * p_LongestReal[Diff + 1 + i + j]) -
                     (p_ShortestImag[LenShortest - 1 - j] * p_LongestImag[Diff + 1 + i + j]);
      SumProdImag += (p_ShortestReal[LenShortest - 1 - j] * p_LongestImag[Diff + 1 + i + j]) +
