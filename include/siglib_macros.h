...
// values Macros that return type SLFixData_t
...
#    define SAI_Absolute(a) \
      (((a) > SIGLIB_ZERO) ? (SAI_RoundToNearest((SLData_t)a)) : (-(SAI_RoundToNearest((SLData_t)a))))    // Returns the absolute value of a
...
#    define SDS_dBmToVoltageMacro(dBm, ZerodBmLevel) \
      (ZerodBmLevel * SDS_Pow(SIGLIB_TEN, (dBm / SIGLIB_TWENTY)))                       // Converts dBm to linear voltage
...
#    define SDS_Swap(a, b)                 \
      {                                    \
        SLData_t SIGLIB_Tmp = (SLData_t)a; \
        a = b;                             \
        b = SIGLIB_Tmp;                    \
      }
#    define SDS_Swap16(a, b)                 \
      {                                      \
        SLInt16_t SIGLIB_Tmp = (SLInt16_t)a; \
        a = b;                               \
        b = SIGLIB_Tmp;                      \
      }
#    define SDS_Swap32(a, b)                 \
      {                                      \
        SLInt32_t SIGLIB_Tmp = (SLInt32_t)a; \
        a = b;                               \
        b = SIGLIB_Tmp;                      \
      }

#    define SDS_Swap2(a, b) \
      {                     \
        a ^= b;             \
        b ^= a;             \
        a ^= b;             \
...
#    define SDS_Sort2(a, b) \
      if ((a) > (b))        \
      SDS_Swap(a, b)

#    define SDS_Sort3(a, b, c) \
      SDS_Sort2(b, c);         \
      SDS_Sort2(a, c);         \
      SDS_Sort2(a, b)

#    define SDS_Sort4(a, b, c, d) \
      SDS_Sort2(a, b);            \
      SDS_Sort2(c, d);            \
      SDS_Sort2(a, c);            \
      SDS_Sort2(b, d)

#    define SDS_Sort5(a, b, c, d, e) \
      SDS_Sort2(a, b);               \
      SDS_Sort2(c, d);               \
      SDS_Sort2(a, c);               \
      SDS_Sort2(a, e);               \
...
#    define SDS_SwapImageData(a, b)                  \
      {                                              \
        SLImageData_t SIGLIB_Tmp = (SLImageData_t)a; \
        a = b;                                       \
        b = SIGLIB_Tmp;                              \
      }

#    define SDS_Sort2ImageData(a, b) \
      if ((a) > (b))                 \
      SDS_SwapImageData(a, b)

#    define SDS_Sort3ImageData(a, b, c) \
      SDS_Sort2ImageData(b, c);         \
      SDS_Sort2ImageData(a, c);         \
      SDS_Sort2ImageData(a, b)

#    define SDS_Sort4ImageData(a, b, c, d) \
      SDS_Sort2ImageData(a, b);            \
      SDS_Sort2ImageData(c, d);            \
      SDS_Sort2ImageData(a, c);            \
      SDS_Sort2ImageData(b, d)

#    define SDS_Sort5ImageData(a, b, c, d, e) \
      SDS_Sort2ImageData(a, b);               \
      SDS_Sort2ImageData(c, d);               \
      SDS_Sort2ImageData(a, c);               \
      SDS_Sort2ImageData(a, e);               \
      SDS_Sort2ImageData(d, e);               \
      SDS_Sort2ImageData(b, e)

#    define SDS_Sort6ImageData(a, b, c, d, e, f) \
      SDS_Sort2ImageData(a, d);                  \
      SDS_Sort2ImageData(b, e);                  \
      SDS_Sort2ImageData(c, f);                  \
      SDS_Sort2ImageData(a, b);                  \
      SDS_Sort2ImageData(a, c);                  \
      SDS_Sort2ImageData(e, f);                  \
      SDS_Sort2ImageData(d, f)
...
#    define SDA_SignalGenerateImpulse(Address, Peak, ArrayLength)                                                                         \
      {                                                                                                                                   \
        SLData_t sl_Dummy;                                                                                                                \
        SDA_SignalGenerate(Address, IMPULSE, Peak, SIGLIB_FILL, SIGLIB_ZERO, SIGLIB_ZERO, SIGLIB_ZERO, SIGLIB_ZERO, &sl_Dummy, &sl_Dummy, \
                           ArrayLength);                                                                                                  \
      }
...
#    define SDA_SignalGenerateKronekerDeltaFunction(Address, Peak, Delay, ArrayLength)                                                            \
      {                                                                                                                                           \
        SLData_t sl_Dummy;                                                                                                                        \
        SDA_SignalGenerate(Address, IMPULSE, Peak, SIGLIB_FILL, SIGLIB_ZERO, SIGLIB_ZERO, Delay, SIGLIB_ZERO, &sl_Dummy, &sl_Dummy, ArrayLength); \
      }

#    define SDA_SignalGenerateWhiteNoise(Address, Peak, Fill_Add, ArrayLength)                                                                    \
      {                                                                                                                                           \
        SLData_t sl_Dummy;                                                                                                                        \
        SDA_SignalGenerate(Address, SIGLIB_WHITE_NOISE, Peak, Fill_Add, SIGLIB_ZERO, SIGLIB_ZERO, SIGLIB_ZERO, SIGLIB_ZERO, &sl_Dummy, &sl_Dummy, \
                           ArrayLength);                                                                                                          \
      }
...
#    define SCV_CopyMacro(IVect, OVect) \
      {                                 \
        OVect.real = IVect.real;        \
...
#    define SUF_FftCoefficientAllocate(a) \
      ((SLData_t*)SUF_MemoryAllocate(((3 * ((size_t)a)) >> 2) * sizeof(SLData_t)))    // FFT twiddle factor coefficient array
#    define SUF_FftCoefficientAllocate4(a) \
      ((SLData_t*)SUF_MemoryAllocate(((5 * ((size_t)a)) >> 2) * sizeof(SLData_t)))    // FFT twiddle factor coefficient array
#    define SUF_FirExtendedArrayAllocate(a) \
      ((SLData_t*)SUF_MemoryAllocate((2 * ((size_t)a)) * sizeof(SLData_t)))    // FIR extended state array filter state array
#    define SUF_IirStateArrayAllocate(a) ((SLData_t*)SUF_MemoryAllocate((2 * ((size_t)a)) * sizeof(SLData_t)))    // IIR filter state array
#    define SUF_IirCoefficientAllocate(a) \
      ((SLData_t*)SUF_MemoryAllocate((((size_t)SIGLIB_IIR_COEFFS_PER_BIQUAD) * ((size_t)a)) * sizeof(SLData_t)))    // IIR filter coefficients array
...
#    define SUF_ComplexRectArrayAllocate(a) \
      ((SLComplexRect_s*)SUF_MemoryAllocate(((size_t)a) * sizeof(SLComplexRect_s)))    // Rectangular complex variable array
#    define SUF_ComplexPolarArrayAllocate(a) \
      ((SLComplexPolar_s*)SUF_MemoryAllocate(((size_t)a) * sizeof(SLComplexPolar_s)))    // Polar complex variable array
#    define SUF_MicrophoneArrayAllocate(a) ((SLMicrophone_s*)SUF_MemoryAllocate(((size_t)a) * sizeof(SLMicrophone_s)))    // Microphone array
#    define SUF_DifferentialEncoderArrayAllocate(a)                          \
      ((SLArrayIndex_t*)SUF_MemoryAllocate(((size_t)((1 << a) * (1 << a))) * \
                                           sizeof(SLFixData_t)))    // Differential encoder / decoder look-up-table

#    define SUF_VectorArrayAllocate16(a) ((SLInt16_t*)SUF_MemoryAllocate(((size_t)a) * sizeof(SLInt16_t)))    // Vector array
#    define SUF_FftCoefficientAllocate16(a) \
      ((SLInt16_t*)SUF_MemoryAllocate(((3 * ((size_t)a)) >> 2) * sizeof(SLInt16_t)))    // FFT twiddle factor coefficient array
#    define SUF_FftCoefficientAllocate416(a) \
      ((SLInt16_t*)SUF_MemoryAllocate(((5 * ((size_t)a)) >> 2) * sizeof(SLInt16_t)))    // FFT twiddle factor coefficient array
#    define SUF_FirExtendedArrayAllocate16(a) \
      ((SLInt16_t*)SUF_MemoryAllocate((2 * ((size_t)a)) * sizeof(SLInt16_t)))    // FIR extended state array filter state array
#    define SUF_IirStateArrayAllocate16(a) ((SLInt16_t*)SUF_MemoryAllocate((2 * ((size_t)a)) * sizeof(SLInt16_t)))    // IIR filter state array
#    define SUF_IirCoefficientAllocate16(a)                                                    \
      ((SLInt16_t*)SUF_MemoryAllocate((((size_t)SIGLIB_IIR_COEFFS_PER_BIQUAD) * ((size_t)a)) * \
                                      sizeof(SLInt16_t)))    // IIR filter coefficients array
...
#    define SUF_CostasLoopVCOArrayAllocate16(a) \
      ((SLInt16_t*)SUF_MemoryAllocate(((5 * ((size_t)a)) >> 2) * sizeof(SLInt16_t)))                                            // QAM carrier array
...
#    define SUF_QPSKCarrierArrayAllocate16(a) \
      ((SLInt16_t*)SUF_MemoryAllocate(((5 * ((size_t)a)) >> 2) * sizeof(SLInt16_t)))    // QPSK carrier array
#    define SUF_OPSKCarrierArrayAllocate16(a) \
      ((SLInt16_t*)SUF_MemoryAllocate(((5 * ((size_t)a)) >> 2) * sizeof(SLInt16_t)))                                       // QPSK carrier array
...
#    define SUF_ComplexRectArrayAllocate16(a) \
      ((SLComplexRect16_s*)SUF_MemoryAllocate(((size_t)a) * sizeof(SLComplexRect16_s)))    // Rectangular complex variable array
#    define SUF_ComplexPolarArrayAllocate16(a) \
      ((SLComplexPolar16_s*)SUF_MemoryAllocate(((size_t)a) * sizeof(SLComplexPolar16_s)))    // Polar complex variable array
...
#    define SUF_DifferentialEncoderArrayAllocate16(a) \
      ((SLArrayIndex_t*)SUF_MemoryAllocate(((size_t)((1 << a) * (1 << a))) * sizeof(SLInt16_t)))    // Differential encoder / decoder look-up-table

#    define SUF_VectorArrayAllocate32(a) ((SLInt32_t*)SUF_MemoryAllocate(((size_t)a) * sizeof(SLInt32_t)))    // Vector array
#    define SUF_FftCoefficientAllocate32(a) \
      ((SLInt32_t*)SUF_MemoryAllocate(((3 * ((size_t)a)) >> 2) * sizeof(SLInt32_t)))    // FFT twiddle factor coefficient array
#    define SUF_FftCoefficientAllocate432(a) \
      ((SLInt32_t*)SUF_MemoryAllocate(((5 * ((size_t)a)) >> 2) * sizeof(SLInt32_t)))    // FFT twiddle factor coefficient array
#    define SUF_FirExtendedArrayAllocate32(a) \
      ((SLInt32_t*)SUF_MemoryAllocate((2 * ((size_t)a)) * sizeof(SLInt32_t)))    // FIR extended state array filter state array
#    define SUF_IirStateArrayAllocate32(a) ((SLInt32_t*)SUF_MemoryAllocate((2 * ((size_t)a)) * sizeof(SLInt32_t)))    // IIR filter state array
#    define SUF_IirCoefficientAllocate32(a)                                                    \
      ((SLInt32_t*)SUF_MemoryAllocate((((size_t)SIGLIB_IIR_COEFFS_PER_BIQUAD) * ((size_t)a)) * \
                                      sizeof(SLInt32_t)))    // IIR filter coefficients array
...
#    define SUF_CostasLoopVCOArrayAllocate32(a) \
      ((SLInt32_t*)SUF_MemoryAllocate(((5 * ((size_t)a)) >> 2) * sizeof(SLInt32_t)))                                            // QAM carrier array
...
#    define SUF_QPSKCarrierArrayAllocate32(a) \
      ((SLInt32_t*)SUF_MemoryAllocate(((5 * ((size_t)a)) >> 2) * sizeof(SLInt32_t)))    // QPSK carrier array
#    define SUF_OPSKCarrierArrayAllocate32(a) \
      ((SLInt32_t*)SUF_MemoryAllocate(((5 * ((size_t)a)) >> 2) * sizeof(SLInt32_t)))                                       // QPSK carrier array
...
#    define SUF_ComplexRectArrayAllocate32(a) \
      ((SLComplexRect32_s*)SUF_MemoryAllocate(((size_t)a) * sizeof(SLComplexRect32_s)))    // Rectangular complex variable array
#    define SUF_ComplexPolarArrayAllocate32(a) \
      ((SLComplexPolar32_s*)SUF_MemoryAllocate(((size_t)a) * sizeof(SLComplexPolar32_s)))    // Polar complex variable array
...
#    define SUF_DifferentialEncoderArrayAllocate32(a) \
      ((SLArrayIndex_t*)SUF_MemoryAllocate(((size_t)((1 << a) * (1 << a))) * sizeof(SLInt32_t)))    // Differential encoder / decoder look-up-table
...
#    define SUF_Debug1(Str1, Var1) \
      {                            \
        printf("%s = ", Str1);     \
        printf(" = %lf\n", Var1);  \
        _getch();                  \
      }
#    define SUF_Debug2(Str1, Var1, Str2, Var2) \
      {                                        \
        printf("%s = ", Str1);                 \
        printf("%lf\n", Var1);                 \
        printf("%s = ", Str2);                 \
...
#    define SUF_Debug3(Str1, Var1, Str2, Var2, Str3, Var3) \
      {                                                    \
        printf("%s = ", Str1);                             \
        printf(" = %lf\n", Var1);                          \
        printf("%s = ", Str2);                             \
...
#    define SUF_Debug4(Str1, Var1, Str2, Var2, Str3, Var3, Str4, Var4) \
      {                                                                \
        printf("%s = ", Str1);                                         \
        printf(" = %lf\n", Var1);                                      \
        printf("%s = ", Str2);                                         \
