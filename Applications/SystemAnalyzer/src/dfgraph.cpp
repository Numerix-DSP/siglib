...
/*
    ...
    Description : Child widget that is
 * used to display the filter graphs
    Notes :
*/
...
/*
    ...
    Description :
 * Function to process when graph type has changed
    Notes :
*/
...
void GraphDisplay::SetDataSetSize(int size)
{
  ...
  //  wxMessageBox (_T("Got here"), "GraphDisplay::SetDataSetSize", wxOK |
  //  wxICON_EXCLAMATION, NULL);
  DataSetSize = size;
  DataGraph->SetStatusBarIndexMultiplier(((double)DataSetSize) / ((double)MAXIMUM_GRAPH_WIDTH));
}
...
void GraphDisplay::EnableGraphTypeComboBox(bool enable)
{
  ...
  //  GraphTypeComboBox->Enable (enable);             // Enable graph type
  //  selected
