...

{
    ...
    if ((gMax-gMin) > 1.0) {                                    // dB plot
        fprintf (plotHandle->pipe, "set for [i=.1:numticks] label at first (f_maxGain-f_minGain)*0.001, first (f_maxGain-f_minGain)*((i/f_numticks) + 0.007) sprintf(\"%%d dB\", (f_minGain-f_maxGain)+(i*10))\n");
    }
    else {                                                      // Linear plot
        fprintf (plotHandle->pipe, "set for [i=.1:numticks] label at first (f_minGain-f_maxGain)*0.001, first (f_maxGain-f_minGain)*((i/f_numticks) + 0.007) sprintf(\"%%1.1f\", 1.+(f_minGain-f_maxGain)+(i*.2))\n");
    }
...
h_GPC_Plot *gpc_plot_confusion_matrix (const double *confusionMatrix,
    const double maxValue,
    const int numCategories)
...
h_GPC_Plot *gpc_plot_confusion_matrix_percentage (const double *confusionMatrix,
    const int numCategories)
