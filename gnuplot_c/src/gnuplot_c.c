...
Please ensure that the system path includes an entry for the gnuplot binary
folder
...
h_GPC_Plot* gpc_init_2d(const char* plotTitle, const char* xLabel, const char* yLabel, const double scalingMode,
                        const enum gpcPlotSignMode signMode, const enum gpcKeyMode keyMode)
{
  ...
  if (NULL == plotHandle->pipe) {
    ...
    printf("Please ensure you have installed Gnuplot from "
...
h_GPC_Plot* gpc_init_2d_logscalex(const char* plotTitle, const char* xLabel, const char* yLabel, const double scalingMode,
                                  const enum gpcPlotSignMode signMode, const enum gpcKeyMode keyMode)
{
  h_GPC_Plot* plotHandle;    // Create plot
...
int gpc_plot_2d(h_GPC_Plot* plotHandle, const double* pData, const int graphLength, const char* pDataName, const double xMin, const double xMax,
                const char* plotType, const char* pColour, const enum gpcNewAddGraphMode addMode)
{
  ...
  fprintf(plotHandle->pipe, "plot $DATA%d u 1:2 t \"%s\" w %s", 0, plotHandle->graphArray[0].title,
          plotHandle->graphArray[0].formatString);           // Send start of plot and first plot command
  for (int i = 1; i <= plotHandle->numberOfGraphs; i++) {    // Send individual plot commands
    fprintf(plotHandle->pipe, ", \\\n $DATA%d u 1:2 t \"%s\" w %s", i, plotHandle->graphArray[i].title,
            plotHandle->graphArray[i].formatString);    // Set plot format
  }
...
#if GPC_DEBUG
  mssleep(100);    // Slow down output so that pipe doesn't overflow when logging
                   // results
#endif
...
h_GPC_Plot* gpc_init_2d_dual_plot(const char* plotTitle, const enum gpcKeyMode keyMode)
{
  h_GPC_Plot* plotHandle;    // Create plot
...
int gpc_plot_2d_dual_plot(h_GPC_Plot* plotHandle, const char* xLabel, const double xMin, const double xMax, const double* pData1,
                          const char* pDataName1, const char* plotType1, const char* pColour1, const char* yLabel1, const double scalingMode1,
                          const enum gpcPlotSignMode signMode1, const double* pData2, const char* pDataName2, const char* plotType2,
                          const char* pColour2, const char* yLabel2, const double scalingMode2, const enum gpcPlotSignMode signMode2,
                          const int graphLength)
{
...
h_GPC_Plot* gpc_init_3d(const char* plotTitle, const char* xLabel, const char* yLabel, const char* zLabel,
...
int gpc_plot_3d(h_GPC_Plot* plotHandle, const double* pX, const double* pY, const double* pZ, const int graphLength, const char* pDataName,
                const char* plotType, const char* pColour, const enum gpcNewAddGraphMode addMode)
{
  if (addMode == GPC_NEW) {                                    // GPC_NEW
    ...
    } else if (plotHandle->scalingMode != GPC_AUTO_SCALE) {    // Set the individual axis scaling to the
                                                               // scalingMode value
  ...
  fprintf(plotHandle->pipe, "splot $DATA%d u 1:2:3 t \"%s\" w %s", 0, plotHandle->graphArray[0].title,
          plotHandle->graphArray[0].formatString);           // Send start of plot and first plot command
  for (int i = 1; i <= plotHandle->numberOfGraphs; i++) {    // Send individual plot commands
    fprintf(plotHandle->pipe, ", \\\n $DATA%d u 1:2:3 t \"%s\" w %s", i, plotHandle->graphArray[i].title,
            plotHandle->graphArray[i].formatString);    // Set plot format
  }
  fprintf(plotHandle->pipe, "\n");    // Send end of plot command
...
h_GPC_Plot* gpc_init_xy(const char* plotTitle, const char* xLabel, const char* yLabel, const double dimension, const enum gpcKeyMode keyMode)
{
  h_GPC_Plot* plotHandle;    // Create plot
...
int gpc_plot_xy(h_GPC_Plot* plotHandle, const ComplexRect_s* pData, const int graphLength, const char* pDataName, const char* plotType,
                const char* pColour, const enum gpcNewAddGraphMode addMode)
{
  if (addMode == GPC_NEW) {
...
h_GPC_Plot* gpc_init_pz(const char* plotTitle, const double dimension, const enum gpcKeyMode keyMode)
{
  h_GPC_Plot* plotHandle;    // Create plot
  ...
  fprintf(plotHandle->pipe,
          "set style line 1 lc rgb \"#ff0000\" lt 1 lw 1   pt 2 ps 1.5\n");    // Poles
                                                                               // - Red
  fprintf(plotHandle->pipe,
          "set style line 2 lc rgb \"#0000ff\" lt 1 lw 1   pt 6 ps 1.5\n");    // Zeros
                                                                               // -
                                                                               // Blue
  fprintf(plotHandle->pipe, "set style line 3 lc rgb \"#000000\" lt 1 lw 0.5 "
                            "pt 5 ps 1.5\n");    // Unit circle - Black
...
int gpc_plot_pz(h_GPC_Plot* plotHandle, const ComplexRect_s* pData, const int graphLength, const char* pDataName,
                const enum gpcPoleZeroMode poleZeroMode, const enum gpcNewAddGraphMode addMode)
{
  if (addMode == GPC_NEW) {
    if (plotHandle->scalingMode == GPC_AUTO_SCALE) {    // Set the X and Y axis scaling
...
h_GPC_Plot* gpc_init_spectrogram(const char* plotTitle, const char* xLabel, const char* yLabel, const int xAxisLength, const int yAxisLength,
                                 const double yMin, const double yMax, const double zMin, const double zMax, const char* pColourPalette,
                                 const enum gpcKeyMode keyMode)
{
  h_GPC_Plot* plotHandle;    // Create plot
  ...
  fprintf(plotHandle->pipe, "set yrange [%1.6le:%1.6le]\n", yMin - (0.5 * ((yMax - yMin) / yAxisLength)),
          yMax + (0.5 * ((yMax - yMin) / yAxisLength)));    // Set length of Y axis
  fprintf(plotHandle->pipe, "set zrange [%1.6le:%1.6le] noreverse nowriteback\n", zMin, zMax);

  fprintf(plotHandle->pipe,
          "set tics out nomirror scale 0.75\n");    // Tics format
...
int gpc_plot_spectrogram(h_GPC_Plot* plotHandle, const double* pData, const char* pDataName, const double xMin, const double xMax)
{
  ...
  if (plotHandle->numberOfGraphs == 0) {
    fprintf(plotHandle->pipe, "set xrange [%1.6le:%1.6le]\n", xMin - (0.5 * ((xMax - xMin) / plotHandle->xAxisLength)),
            xMax + (0.5 * ((xMax - xMin) / plotHandle->xAxisLength)));    // Set length of X axis
...
h_GPC_Plot* gpc_init_image(const char* plotTitle, const int xAxisLength, const int yAxisLength, const unsigned int zMin, const unsigned int zMax,
                           const char* pColourPalette, const enum gpcKeyMode keyMode)
{
  h_GPC_Plot* plotHandle;    // Create plot
...
int gpc_plot_image(h_GPC_Plot* plotHandle, const unsigned int* pData, const char* pDataName)
{
...
h_GPC_Plot* gpc_init_polar(const char* plotTitle, const double gMin, const double gMax, const enum gpcKeyMode keyMode)
{
  h_GPC_Plot* plotHandle;    // Create plot
  ...
  if ((gMax - gMin) > 1.0) {                               // dB plot
    ...
    fprintf(plotHandle->pipe, "numticks = %u\n",
            (unsigned)((gMax - gMin) / 10.));    // # numticks = r / tickstep :: Don't
                                                 // use divide because numticks is used
                                                 // in for loop and doesn't work
    fprintf(plotHandle->pipe, "f_numticks = %lf\n",
            (gMax - gMin) / 10.);                    // # Floating point numticks
  } else {                                           // Linear plot
    fprintf(plotHandle->pipe, "tickstep = .2\n");    // # Ticks every .1
    fprintf(plotHandle->pipe, "numticks = %u\n",
            (unsigned)((gMax - gMin) / .2));    // # numticks = r / tickstep :: Don't
                                                // use divide because numticks is used
                                                // in for loop and doesn't work
    fprintf(plotHandle->pipe, "f_numticks = %lf\n",
  ...
  fprintf(plotHandle->pipe, "set label '180°' centre at first 0, first "
                            "-(f_maxGain-f_minGain)*1.05\n");
  fprintf(plotHandle->pipe, "set label '270°' left at first -(f_maxGain-f_minGain)*1.05, 0\n");

  if ((gMax - gMin) > 1.0) {    // dB plot
    fprintf(plotHandle->pipe, "set for [i=.1:numticks] label at first (f_maxGain-f_minGain)*0.001, "
                              "first (f_maxGain-f_minGain)*((i/f_numticks) + 0.007) sprintf(\"%%d "
                              "dB\", (f_minGain-f_maxGain)+(i*10))\n");
  } else {    // Linear plot
    fprintf(plotHandle->pipe, "set for [i=.1:numticks] label at first (f_minGain-f_maxGain)*0.001, "
                              "first (f_maxGain-f_minGain)*((i/f_numticks) + 0.007) "
                              "sprintf(\"%%1.1f\", 1.+(f_minGain-f_maxGain)+(i*.2))\n");
  }
...
int gpc_plot_polar(h_GPC_Plot* plotHandle, const double* pAngles, const double* pGains, const int numAngles, const char* pDataName,
                   const char* plotType, const char* pColour, const enum gpcNewAddGraphMode addMode)
{
  if (addMode == GPC_NEW) {    // GPC_NEW
    plotHandle->numberOfGraphs = 0;
  ...
  fprintf(plotHandle->pipe, "plot $DATA%d u (-$1+90.):($2-f_maxGain) t \"%s\" w %s", 0, plotHandle->graphArray[0].title,
          plotHandle->graphArray[0].formatString);           // Send start of plot and first plot command
  for (int i = 1; i <= plotHandle->numberOfGraphs; i++) {    // Send individual plot commands
    fprintf(plotHandle->pipe, ", \\\n $DATA%d u (-$1+90.):($2-f_maxGain) t \"%s\" w %s", i, plotHandle->graphArray[i].title,
            plotHandle->graphArray[i].formatString);    // Set plot format
  }
  fprintf(plotHandle->pipe, "\n");    // Send end of plot command
...
h_GPC_Plot* gpc_plot_confusion_matrix(const double* confusionMatrix, const double maxValue, const int numCategories)
{
  h_GPC_Plot* plotHandle;    // Create plot
  ...
  if (NULL == plotHandle->pipe) {
    ...
    fprintf(plotHandle->pipe, "plot $confMat matrix using 1:2:3 with image, $confMat matrix "
...
h_GPC_Plot* gpc_plot_confusion_matrix_percentage(const double* confusionMatrix, const int numCategories)
{
  h_GPC_Plot* plotHandle;    // Create plot
  ...
  if (NULL == plotHandle->pipe) {
    ...
    fprintf(plotHandle->pipe, "set palette rgbformulae -7, -7, 2\n");
    fprintf(plotHandle->pipe, "plot $confMat matrix using 1:2:3 with image, $confMat matrix "
                              "using 1:2:( sprintf(\"%%1.1f %%\",$3) ) with labels\n");
