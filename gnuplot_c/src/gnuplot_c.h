...
#  define GPC_AUTO_SCALE \
    0.0                                 // Auto scaling - In 3D plot, each axis is individually scaled to their
                                        // maximum
...
// static const char* gpcPlotColours[] = {"purple", "green", "blue", "black", "orange", "cyan", "violet", "grey", "magenta", "light-red"};
// The following colours match Matplotlib defaults
static const char* gpcPlotColours[] = {"#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd",
...
typedef struct {
  ...
  int numberOfGraphs;                        // Number of graphs on this plot or Number of columns for
                                             // spectrogram plot
...
int gpc_plot_2d(h_GPC_Plot* plotHandle,                    // Plot handle
                ...
                const char* plotType,                      // Plot type - "lines", "points",
                                                           // "impulses", "linespoints"
                const char* pColour,                       // Colour - Use gnuplot> show colornames to
...
int gpc_plot_2d_dual_plot(h_GPC_Plot* plotHandle,                  // Plot handle
                          ...
                          const char* plotType1,                   // Dataset #1 Plot type - "lines", "points",
                                                                   // "impulses", "linespoints"
                          const char* pColour1,                    // Dataset #1 Colour - Use gnuplot> show colornames to
                                                                   // see available colours
                          ...
                          const char* plotType2,                   // Dataset #2 Plot type - "lines", "points",
                                                                   // "impulses", "linespoints"
                          const char* pColour2,                    // Dataset #2 Colour - Use gnuplot> show colornames to
                                                                   // see available colours
...
h_GPC_Plot* gpc_init_3d(const char* plotTitle,             // Plot title
                        const char* xLabel,                // X axis label
                        const char* yLabel,                // y axis label
...
int gpc_plot_3d(h_GPC_Plot* plotHandle,                    // Plot handle
...
int gpc_plot_polar(h_GPC_Plot* plotHandle,                    // Plot handle
                   ...
                   const char* plotType,                      // Plot type - "lines", "points",
                                                              // "impulses", "linespoints"
                   const char* pColour,                       // Colour - Use gnuplot> show colornames
                                                              // to see available colours
                   const enum gpcNewAddGraphMode addMode);    // Add / new mode

h_GPC_Plot* gpc_plot_confusion_matrix(const double*,    // Pointer to source data
                                      const double,     // Maximum value to scale matrix colourization
                                      const int);       // Number of categories

h_GPC_Plot* gpc_plot_confusion_matrix_percentage(const double*,    // Pointer to source data
                                                 const int);       // Number of categories
...
#    include <windows.h>
#    if (GPC_DEBUG == 1)
                                           // #define GNUPLOT_CMD "gnuplot" // Do
                                           // not pipe the text output to null so
...
#    if (GPC_DEBUG == 1)
#      define GNUPLOT_CMD \
        "tee debug.log | gnuplot -persist"    // Do not pipe the text output to null so
                                              // that it can be used for debugging
                                              // #define GNUPLOT_CMD "gnuplot >
                                              // debug.log"          // Pipe the
                                              // text output to debug.log for
                                              // debugging
#    else
#      if ((__unix) || (__MACH__))    // Linux / OS X
#        define GNUPLOT_CMD \
          "gnuplot > /dev/null 2>&1"    // Pipe the text output to null for higher
                                        // performance
#      else
#        define GNUPLOT_CMD \
          "gnuplot > /dev/nul 2>&1"    // Pipe the text output to null for higher
                                       // performance
#      endif
#    endif
#    ifndef GPC_TERM
...
#    endif
#    ifndef GPC_TERM
//        #define GPC_TERM    "qt"                            // Gnuplot term
//        type = "qt". Most Linux installations use "wxt" now.
#      define GPC_TERM "wxt"    // Gnuplot term type
#    endif
#    define mssleep(u) usleep(u * 1000)
#  endif
